...
const QUESTIONS = [
  { id: 1, question: 'What makes instructions most effective?', options: ['Clarity and sequencing', 'Using complex vocabulary', 'Being very brief', 'Avoiding politeness'], correct: 0 },
  { id: 2, question: 'Which best demonstrates professional instruction?', options: ['First, systematically verify credentials.', 'Check ticket.', 'Do this now.', 'Maybe check?'], correct: 0 },
  { id: 3, question: 'Why use sequencing words (first, then, after)?', options: ['To create logical order', 'To sound formal', 'To be polite', 'To save time'], correct: 0 },
  { id: 4, question: 'What does "diplomatically" mean in instructions?', options: ['Tactfully and sensitively', 'Quickly and efficiently', 'Loudly and clearly', 'Briefly and directly'], correct: 0 },
  { id: 5, question: 'How should critical incidents be handled?', options: ['Calmly and professionally', 'Ignored if minor', 'Loudly to get attention', 'Passed to someone else'], correct: 0 },
  { id: 6, question: 'What is the purpose of a post-event debrief?', options: ['Evaluate performance and improve', 'Assign blame for mistakes', 'Celebrate achievements only', 'Dismiss volunteers quickly'], correct: 0 }
]
...
export default function Phase5SubPhase2Step2RemedialC1TaskC() {
  ...
  const handleSubmit = async () => {
    ...
    sessionStorage.setItem('phase5_subphase2_step2_remedial_c1_taskC_score', c.toString())
    try { await phase5API.logRemedialActivity(2, 'C1', 'C', c, 6, 2) } catch (e) { console.error(e) }
  }
  const handleContinue = () => navigate('/phase5/subphase/2/step/2/remedial/c1/task/d')
      ...
      <Container maxWidth="md">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}>
          <Box sx={{ ...clay(P.orange), mb: 3 }}>
            ...
            <Typography variant="body1" sx={{ color: theme.palette.text.secondary }}>Answer 6 advanced questions on professional instruction principles.</Typography>
          </Box>
        </motion.div>
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}>
          <Box sx={{ ...clay(P.orange), mb: 3 }}>
            <CharacterMessage speaker="Ms. Mabrouki" message="Welcome to Advanced Quiz! Answer these questions about the principles of professional instruction in high-stakes environments." />
          </Box>
        </motion.div>
        {!submitted ? (
