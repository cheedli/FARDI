...
const SENTENCES = ['[Please] welcome guests.', '[First], check ticket.', '[Then], show way.', 'Be [careful].', '[Help] people.', 'Say [thank you].']
...
export default function Phase5SubPhase2Step2RemedialA2TaskB() {
  ...
  const { saveResponse } = useProgressSave({ phase: 5, subphase: 2, step: 2, interaction: 2, context: 'remedial_a2' })
  ...
  const handleSubmit = async () => {
    ...
    try { await phase5API.logRemedialActivity(2, 'A2', 'B', finalScore, 6, 2) } catch (e) { console.error(e) }
  }

  const handleContinue = () => navigate('/phase5/subphase/2/step/2/remedial/a2/task/c')
      ...
      <Container maxWidth="md">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}>
          <Box sx={{ ...clay(P.orange), mb: 3 }}>
            ...
            <Typography variant="body1" sx={{ color: theme.palette.text.secondary }}>Fill in 6 gaps with instruction words. Fill fast for high score!</Typography>
          </Box>
        </motion.div>
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}>
          <Box sx={{ ...clay(P.orange), mb: 3 }}>
            <CharacterMessage speaker="Ms. Mabrouki" message="Welcome to Fill Frenzy! Fill in the gaps with words from the word bank. Complete all sentences to finish!" />
          </Box>
        </motion.div>
        {!submitted && (
        ...
        {submitted && (
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}>
            <Box sx={{ ...clay(P.green), mb: 3, textAlign: 'center' }}>
              ...
              <Typography variant="body1" sx={{ mt: 1 }}>{score >= 6 ? 'Excellent! All correct!' : "Good work! Let's continue."}</Typography>
            </Box>
