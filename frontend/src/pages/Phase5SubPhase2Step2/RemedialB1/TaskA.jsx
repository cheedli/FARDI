...
const SENTENCES = ['SKANDER: What do first?', 'You: [First], [please] welcome guests.', 'Emna: Then?', 'You: [Then], check ticket and [help] them.']
const CORRECT_ANSWERS = { 'g_1_0': 'First', 'g_1_1': 'please', 'g_3_0': 'Then', 'g_3_1': 'help' }
...
export default function Phase5SubPhase2Step2RemedialB1TaskA() {
  ...
  const handleSubmit = async () => {
    ...
    try { await phase5API.logRemedialActivity(2, 'B1', 'A', finalScore, 4, 2) } catch (e) { console.error(e) }
  }
  const handleContinue = () => navigate('/phase5/subphase/2/step/2/remedial/b1/task/b')
      ...
      <Container maxWidth="md">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}>
          <Box sx={{ ...clay(P.orange), mb: 3 }}>
            ...
            <Typography variant="body1" sx={{ color: theme.palette.text.secondary }}>Fill gaps in dialogue giving volunteer instructions.</Typography>
          </Box>
        </motion.div>
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}>
          <Box sx={{ ...clay(P.orange), mb: 3 }}>
            <CharacterMessage speaker="Ms. Mabrouki" message="Welcome to Negotiation Simulator! Complete the dialogue about giving volunteer instructions by filling in the gaps. Use the word bank!" />
          </Box>
        </motion.div>
        {!submitted && (
