...
export default function Phase6SP1Step1RemB1TaskC() {
  ...
  React.useEffect(() => { resolveSubphase1RemedialNextUrl(1, 'B1').then(url => { window.__remedialSkip = () => navigate(url) }) }, [])
        ...
        {!submitted ? (
          ...
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}>
            <Box sx={{
              ...
              <Box
                component="button"
                onClick={async () => navigate(await resolveSubphase1RemedialNextUrl(1, 'B1'))}
                sx={{
                  cursor: 'pointer',
                  mt: 2,
                  px: 4,
