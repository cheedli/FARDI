...
export default function Phase5SubPhase2Step1RemedialA2TaskB() {
  ...
  const handleSubmit = async () => {
    ...
    try { await phase5API.logRemedialActivity(1, 'A2', 'B', finalScore, 6, 0, 2) } catch (e) { console.error(e) }
  }

  const handleContinue = () => navigate('/phase5/subphase/2/step/1/remedial/a2/task/c')
        ...
        {submitted && (
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}>
            <Box sx={{ ...clay(P.green), mb: 3, textAlign: 'center' }}>
              ...
              <Typography variant="body1" sx={{ mt: 1 }}>{score >= 6 ? 'Excellent! All correct!' : "Good work! Let's continue to the next task."}</Typography>
            </Box>
