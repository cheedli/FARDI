...
export default function Phase5SubPhase2Step1ScoreCalculation() {
  ...
  const handleContinue = () => {
    if (!routing) return
    if (routing.shouldProceed) navigate('/phase5/subphase/2/step/2')
    else navigate(`/phase5/subphase/2/step/1/remedial/${routing.remedialLevel.toLowerCase()}/task/a`)
  }
            ...
            <Box sx={{ ...clay(routing.shouldProceed ? P.green : P.orange), mb: 3 }}>
              ...
              <Box sx={{ bgcolor: routing.shouldProceed ? P.green.bg : P.orange.bg, border: `1px solid ${routing.shouldProceed ? P.green.border : P.orange.border}`, borderRadius: '10px', p: 2, mb: 2 }}>
                {routing.shouldProceed ? (
                  <Typography variant="body1">Great work! You've demonstrated strong understanding. You can proceed to Step 2 or complete remedial activities for extra practice.</Typography>
                ) : (
