...
export default function Phase5SubPhase2Step1RemedialC1TaskB() {
  ...
  const { saveResponse } = useProgressSave({ phase: 5, subphase: 2, step: 1, interaction: 2, context: 'remedial_c1' })
  ...
  const handleSubmit = async () => {
    ...
    try { await phase5API.logRemedialActivity(1, 'C1', 'B', correctCount, 8, 2) } catch (e) { console.error(e) }
  }

  const handleContinue = () => navigate('/phase5/subphase/2/step/1/remedial/c1/task/c')
      ...
      <Container maxWidth="md">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}>
          <Box sx={{ ...clay(P.orange), mb: 3 }}>
            ...
            <Typography variant="body1" sx={{ color: theme.palette.text.secondary }}>Write an 8-sentence set of professional instructions for volunteers. Show professional, empathetic, well-structured writing!</Typography>
          </Box>
        </motion.div>
        ...
        {!submitted ? (
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
            ...
            <Box sx={{ ...clay(P.blue), mb: 3 }}>
              ...
              <TextField fullWidth multiline rows={4} variant="outlined" placeholder="Write your professional sentence here..."
