...
export default function Phase6SP1Step5RemB2TaskC() {
  ...
  React.useEffect(() => { resolveSubphase1RemedialNextUrl(5, 'B2').then(url => { window.__remedialSkip = () => navigate(url) }) }, [])
        ...
        {!submitted ? (
          ...
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}>
            <Box sx={{ ...cardSx('green'), textAlign: 'center' }}>
              ...
              <Box
                component="button"
                onClick={async () => navigate(await resolveSubphase1RemedialNextUrl(5, 'B2'))}
                sx={{
                  mt: 2, px: 6, py: 1.5,
                  bgcolor: P.green.bg,
