...
import { Box, Container, Typography, TextField, Stack, CircularProgress } from '@mui/material'
      ...
      <Container maxWidth="lg">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}>
          <Box sx={{ ...cardSx('blue'), mb: 3 }}>
            ...
            <Typography variant="body1" color="text.secondary">Grammar Correction</Typography>
          </Box>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
          <Box sx={{ ...cardSx('orange'), mb: 3 }}>
            <Typography variant="body2" sx={{ color: P.orange.border }}>
              <strong>Hint:</strong> Look for verb tense errors, subject-verb agreement, and missing articles.
            </Typography>
          </Box>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}>
          <Box sx={{ ...cardSx('teal'), mb: 3 }}>
            <Typography variant="subtitle2" gutterBottom sx={{ color: P.teal.border, fontWeight: 'bold' }}>Target Vocabulary:</Typography>
            <Stack direction="row" flexWrap="wrap" gap={1}>
              {TARGET_VOCABULARY.map((w, i) => (
                <Box key={i} sx={{
                  ...
                  <Typography variant="body2" sx={{ color: vocabUsed.includes(w) ? 'white' : P.teal.border, fontWeight: 'bold' }}>{w}</Typography>
                </Box>
              ))}
            </Stack>
          </Box>
          ...
          <Box sx={{ ...cardSx('blue'), mb: 3 }}>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
              <Typography variant="caption" color="text.secondary">
                Words: {wordCount} | Vocabulary: {vocabUsed.length}/{TARGET_VOCABULARY.length}
              </Typography>
            </Box>
            {!submitted && (
              <Box
        ...
        {evaluation && (
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}>
            <Box sx={{ ...cardSx('green'), mb: 3 }}>
              <Typography variant="h6" sx={{ color: P.green.border }}>
              ...
              {evaluation.strengths?.length > 0 && (
                <Box sx={{ mb: 2 }}>
                  <Typography variant="subtitle2" sx={{ color: P.green.border }}>Strengths:</Typography>
                  <ul style={{ margin: 0, paddingLeft: 20 }}>
                    {evaluation.strengths.map((s, i) => <li key={i}><Typography variant="body2">{s}</Typography></li>)}
                  </ul>
                </Box>
              )}
              {evaluation.improvements?.length > 0 && (
                <Box sx={{ mb: 2 }}>
                  <Typography variant="subtitle2" sx={{ color: P.orange.border }}>Improvements:</Typography>
                  <ul style={{ margin: 0, paddingLeft: 20 }}>
                    {evaluation.improvements.map((s, i) => <li key={i}><Typography variant="body2">{s}</Typography></li>)}
                  </ul>
                </Box>
              )}
              {submitted && (
                <Box
                  component="button"
                  onClick={() => navigate('/phase6/subphase/1/step/5/interaction/3')}
                  sx={{
                    width: '100%', py: 1.5, mt: 1,
                    bgcolor: P.green.bg,
