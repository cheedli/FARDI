...
export default function Phase5Step3RemedialB2TaskC() {
  ...
  const { saveResponse } = useProgressSave({ phase: 5, subphase: 1, step: 3, interaction: 3, context: 'remedial_b2' })
  ...
  const handleGameComplete = async (result) => {
    ...
    try { await phase5API.logRemedialActivity(3, 'B2', 'C', score, 8, result.timeTaken || 0) } catch (e) { console.error(e) }
  }
      ...
      <Container maxWidth="md">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}>
          <Box sx={{ ...cardSx(P.orange), mb: 3 }}>
            ...
            <Typography variant="h6" gutterBottom sx={{ color: P.orange.border }}>Task C: Kahoot Match</Typography>
            <Typography variant="body1" color="text.secondary">Match 8 terms to advanced definitions</Typography>
          </Box>
        </motion.div>
