...
export default function Phase5Step3RemedialC1TaskD() {
  ...
  const handleSubmit = async () => {
    let correctCount = 0
    critiques.forEach(critique => { if (critique.trim().split(/\s+/).filter(w => w.length > 0).length >= 8) correctCount++ })
    setScore(correctCount)
    setSubmitted(true)
    sessionStorage.setItem('phase5_step3_remedial_c1_taskD_score', correctCount.toString())
    try { await phase5API.logRemedialActivity(3, 'C1', 'D', correctCount, 6, 0) } catch (e) { console.error(e) }
  }
  ...
  const handleContinue = async () => {
    ...
    const passed = totalScore >= Math.ceil((6 + 8 + 6 + 6) * 0.8)
    try { await phase5API.calculateRemedialScore(3, 'C1', { task_a_score: taskAScore, task_b_score: taskBScore, task_c_score: taskCScore, task_d_score: taskDScore }) } catch (e) { console.error(e) }
    navigate(passed ? '/dashboard' : '/phase5/subphase/1/step/3/remedial/c1/task/a')
  }
      ...
      <Container maxWidth="md">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}>
          <Box sx={{ ...cardSx(P.orange), mb: 3 }}>
            ...
            <Typography variant="body1" color="text.secondary">Critique 6 crisis communication elements with nuanced analysis</Typography>
          </Box>
        </motion.div>
          ...
          <>
            <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}>
              <Box sx={{ ...cardSx(P.yellow), mb: 3 }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                  <Typography variant="body2" color="text.secondary">Element {currentIndex + 1} of 6</Typography>
            ...
            <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
              <Box sx={{ ...cardSx(P.blue), mb: 3 }}>
                <Typography variant="h6" gutterBottom fontWeight="bold" sx={{ color: P.blue.border }}>Element: {ELEMENTS[currentIndex].element}</Typography>
                <Box sx={{ ...cardSx(P.teal), mb: 2 }}>
                  <Box sx={{ display: 'flex', alignItems: 'center' }}>
                    <InfoIcon sx={{ color: P.teal.border, mr: 1, fontSize: 18 }} />
                    <Typography variant="body2" sx={{ color: P.teal.border }}><strong>Example critique:</strong> {ELEMENTS[currentIndex].exampleCritique}</Typography>
                  </Box>
                </Box>
