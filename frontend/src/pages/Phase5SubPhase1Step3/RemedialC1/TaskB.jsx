...
export default function Phase5Step3RemedialC1TaskB() {
  ...
  const { saveResponse } = useProgressSave({ phase: 5, subphase: 1, step: 3, interaction: 2, context: 'remedial_c1' })
  ...
  const handleSubmit = async () => {
    let correctCount = 0
    answers.forEach(answer => { if (answer.trim().split(/\s+/).filter(w => w.length > 0).length >= 10) correctCount++ })
    setScore(correctCount)
    setSubmitted(true)
    sessionStorage.setItem('phase5_step3_remedial_c1_taskB_score', correctCount.toString())
    try { await phase5API.logRemedialActivity(3, 'C1', 'B', correctCount, 8, 0) } catch (e) { console.error(e) }
  }

  const handleContinue = () => { navigate('/phase5/subphase/1/step/3/remedial/c1/task/c') }
      ...
      <Container maxWidth="md">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}>
          <Box sx={{ ...cardSx(P.orange), mb: 3 }}>
            ...
            <Typography variant="body1" color="text.secondary">Write an 8-sentence analytical paragraph on crisis communication</Typography>
          </Box>
        </motion.div>
