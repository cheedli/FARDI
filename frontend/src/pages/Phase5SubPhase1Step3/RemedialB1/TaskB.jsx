...
export default function Phase5Step3RemedialB1TaskB() {
  ...
  const handleSubmit = async () => {
    let correctCount = 0
    answers.forEach(answer => { if (answer.trim().split(/\s+/).filter(w => w.length > 0).length >= 5) correctCount++ })
    setScore(correctCount)
    setSubmitted(true)
    sessionStorage.setItem('phase5_step3_remedial_b1_taskB_score', correctCount.toString())
    try { await phase5API.logRemedialActivity(3, 'B1', 'B', correctCount, 8, 0) } catch (e) { console.error(e) }
  }

  const handleContinue = () => { navigate('/phase5/subphase/1/step/3/remedial/b1/task/c') }
      ...
      <Container maxWidth="md">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}>
          <Box sx={{ ...cardSx(P.orange), mb: 3 }}>
            ...
            <Typography variant="h6" gutterBottom sx={{ color: P.orange.border }}>Task B: Definition Duel</Typography>
            <Typography variant="body1" color="text.secondary">Write 8 definitions with examples for crisis terms</Typography>
          </Box>
        </motion.div>
            ...
            <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}>
              <Box sx={{ ...cardSx(P.yellow), mb: 3 }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                  <Typography variant="body2" color="text.secondary">Definition {currentIndex + 1} of 8</Typography>
