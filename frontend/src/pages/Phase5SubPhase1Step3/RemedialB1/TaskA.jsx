...
export default function Phase5Step3RemedialB1TaskA() {
  ...
  const handleSubmit = async () => {
    ...
    try { await phase5API.logRemedialActivity(3, 'B1', 'A', finalScore, 5, 0) } catch (e) { console.error(e) }
  }

  const handleContinue = () => { navigate('/phase5/subphase/1/step/3/remedial/b1/task/b') }
      ...
      <Container maxWidth="md">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}>
          <Box sx={{ ...cardSx(P.orange), mb: 3 }}>
            ...
            <Typography variant="h6" gutterBottom sx={{ color: P.orange.border }}>Task A: Negotiation Battle</Typography>
            <Typography variant="body1" color="text.secondary">Fill gaps explaining crisis terms</Typography>
          </Box>
        </motion.div>
        ...
        {!submitted ? (
          ...
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}>
            <Box sx={{ ...cardSx(P.green), mb: 3, textAlign: 'center' }}>
              ...
              <Typography variant="h6" sx={{ mt: 2 }} color="text.secondary">Score: {score} / 5</Typography>
            </Box>
