...
export default function Phase5Step3RemedialB1TaskC() {
  ...
  const { saveResponse } = useProgressSave({ phase: 5, subphase: 1, step: 3, interaction: 3, context: 'remedial_b1' })
  ...
  const handleSubmit = async () => {
    ...
    try { await phase5API.logRemedialActivity(3, 'B1', 'C', correctCount, 6, 0) } catch (e) { console.error(e) }
  }
  ...
  const handleContinue = async () => {
    ...
    const passed = totalScore >= Math.ceil((5 + 8 + 6) * 0.8)
    try { await phase5API.calculateRemedialScore(3, 'B1', { task_a_score: taskAScore, task_b_score: taskBScore, task_c_score: taskCScore }) } catch (e) { console.error(e) }
    navigate(passed ? '/dashboard' : '/phase5/subphase/1/step/3/remedial/b1/task/a')
  }
      ...
      <Container maxWidth="md">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}>
          <Box sx={{ ...cardSx(P.orange), mb: 3 }}>
            ...
            <Typography variant="body1" color="text.secondary">Answer 6 multiple-choice questions on terms</Typography>
          </Box>
        </motion.div>
