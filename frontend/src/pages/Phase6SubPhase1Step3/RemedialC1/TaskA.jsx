...
const WORD_BANK = ['balanced evaluation', 'transparency', 'continuous improvement', 'evidence-based', 'actionable recommendations']
...
export default function Phase6SP1Step3RemedialC1TaskA() {
  ...
  React.useEffect(() => { window.__remedialSkip = () => navigate('/phase6/subphase/1/step/3/remedial/c1/task/b') }, [])
  ...
  const handleSubmit = async () => {
    ...
    try { await phase6API.logRemedialActivity(3, 'C1', 'A', correct, Object.keys(CORRECT_ANSWERS).length, 0, 1) } catch (e) { console.error(e) }
  }
      ...
      <Container maxWidth="md">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}>
          <Box sx={{ ...cardSx(P.orange), mb: 3 }}>
            ...
            <Typography variant="body1" color="text.secondary">Simulate a dialogue defending the value of a balanced report</Typography>
          </Box>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
          <Box sx={{ ...cardSx(P.green), mb: 3 }}>
            <Typography variant="subtitle1" fontWeight="bold" gutterBottom sx={{ color: P.green.border }}>Advanced Word Bank:</Typography>
        ...
        {!submitted ? (
          ...
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}>
            <Box sx={{ ...cardSx(P.green), textAlign: 'center' }}>
              ...
              <Box component="button" onClick={() => navigate('/phase6/subphase/1/step/3/remedial/c1/task/b')} sx={{ ...cardSx(P.orange), p: 1.5, cursor: 'pointer', fontWeight: 'bold', color: P.orange.border, '&:hover': { transform: 'translate(-2px,-2px)', boxShadow: `6px 6px 0 ${P.orange.shadow}` }, transition: 'all 0.15s ease' }}>
                Next: Task B →
              </Box>
            </Box>
          </motion.div>
