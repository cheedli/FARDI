...
const CORRECT_ANSWERS = { 0: 'good', 1: 'result', 2: 'Feedback', 3: 'improve' }

export default function Phase6SP1Step3RemedialB1TaskA() {
  const navigate = useNavigate()
  React.useEffect(() => { window.__remedialSkip = () => navigate('/phase6/subphase/1/step/3/remedial/b1/task/b') }, [])
  ...
  const handleSubmit = async () => {
    ...
    try { await phase6API.logRemedialActivity(3, 'B1', 'A', correct, Object.keys(CORRECT_ANSWERS).length, 0, 1) } catch (e) { console.error(e) }
  }
      ...
      <Container maxWidth="md">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}>
          <Box sx={{ ...cardSx(P.orange), mb: 3 }}>
            ...
            <Typography variant="body1" color="text.secondary">Fill gaps in a dialogue explaining report terms</Typography>
          </Box>
        </motion.div>
        ...
        {!submitted ? (
          ...
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}>
            <Box sx={{ ...cardSx(P.green), textAlign: 'center' }}>
              ...
              <Box component="button" onClick={() => navigate('/phase6/subphase/1/step/3/remedial/b1/task/b')} sx={{ ...cardSx(P.orange), p: 1.5, cursor: 'pointer', fontWeight: 'bold', color: P.orange.border, '&:hover': { transform: 'translate(-2px,-2px)', boxShadow: `6px 6px 0 ${P.orange.shadow}` }, transition: 'all 0.15s ease' }}>
                Next: Task B →
              </Box>
            </Box>
          </motion.div>
