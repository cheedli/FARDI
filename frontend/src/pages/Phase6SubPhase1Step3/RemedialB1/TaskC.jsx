...
const QUESTIONS = [
  { question: 'What does "success" mean?', options: ['Good result', 'A big problem', 'A list of names'], correct: 0, explanation: 'Success = Good result. When things go well, it is a success!' },
  { question: 'What does "challenge" mean?', options: ['Easy activity', 'Difficult thing', 'Happy feeling'], correct: 1, explanation: 'Challenge = Difficult thing. A challenge is something hard to deal with.' },
  { question: 'What does "feedback" mean?', options: ['A type of music', 'A new game', 'Opinion'], correct: 2, explanation: 'Feedback = Opinion. Feedback is what people think or say about the event.' },
  { question: 'What does "improve" mean?', options: ['Make better', 'Write a list', 'Ask a question'], correct: 0, explanation: 'Improve = Make better. We improve when we do something better than before.' },
  { question: 'What does "strength" mean?', options: ['A weakness', 'Good skill', 'A problem'], correct: 1, explanation: 'Strength = Good skill. A strength is something we are good at.' },
  { question: 'What does "recommend" mean?', options: ['Find an error', 'Write a summary', 'Suggest better'], correct: 2, explanation: 'Recommend = Suggest better. When you recommend, you suggest something for next time.' }
]

export default function Phase6SP1Step3RemedialB1TaskC() {
  const navigate = useNavigate()
  React.useEffect(() => { resolveSubphase1RemedialNextUrl(3, 'B1').then(url => { window.__remedialSkip = () => navigate(url) }) }, [])
  ...
  const handleSubmit = async () => {
    ...
    try { await phase6API.logRemedialActivity(3, 'B1', 'C', correct, QUESTIONS.length, 0, 1) } catch (e) { console.error(e) }
  }
        ...
        {!submitted ? (
          ...
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}>
            <Box sx={{ ...cardSx(P.green), textAlign: 'center' }}>
              ...
              <Box component="button" onClick={async () => navigate(await resolveSubphase1RemedialNextUrl(3, 'B1'))} sx={{ ...cardSx(P.orange), p: 1.5, cursor: 'pointer', fontWeight: 'bold', color: P.orange.border, '&:hover': { transform: 'translate(-2px,-2px)', boxShadow: `6px 6px 0 ${P.orange.shadow}` }, transition: 'all 0.15s ease' }}>
                Continue →
              </Box>
            </Box>
          </motion.div>
