...
export default function Phase6SP1Step3RemedialB2TaskC() {
  ...
  React.useEffect(() => { window.__remedialSkip = () => navigate('/phase6/subphase/1/step/3/remedial/b2/task/d') }, [])
  const theme = useTheme()
  const P = theme.palette.mode === 'dark' ? DARK : LIGHT
  const { saveResponse } = useProgressSave({ phase: 6, subphase: 1, step: 3, interaction: 3, context: 'remedial_b2' })
  ...
  const handleSubmit = async () => {
    let correct = 0
    Object.entries(matches).forEach(([termIdx, defIdx]) => { if (PAIRS[parseInt(termIdx)].term === shuffledDefs[parseInt(defIdx)].term) correct++ })
    setScore(correct)
    setSubmitted(true)
    sessionStorage.setItem('phase6_sp1_step3_remedial_b2_taskc_score', correct.toString())
    try { await phase6API.logRemedialActivity(3, 'B2', 'C', correct, PAIRS.length, 0, 1) } catch (e) { console.error(e) }
  }
      ...
      <Container maxWidth="md">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}>
          <Box sx={{ ...cardSx(P.orange), mb: 3 }}>
            ...
            <Typography variant="body1" color="text.secondary">Match 8 report terms to their advanced definitions</Typography>
          </Box>
        </motion.div>
        ...
        {!submitted ? (
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
            <Box sx={{ ...cardSx(P.teal), mb: 3, p: 2 }}>
              <Typography variant="body2">Click a term (left) to select it, then click its matching definition (right).</Typography>
            </Box>
            <Grid container spacing={3} sx={{ mb: 3 }}>
              <Grid item xs={12} md={5}>
                <Box sx={{ ...cardSx(P.blue), p: 2 }}>
                  <Typography variant="h6" gutterBottom textAlign="center" sx={{ color: P.green.border }}>Terms</Typography>
                  <Stack spacing={1.5}>
                    {PAIRS.map((p, idx) => (
                      <Box key={idx} onClick={() => handleTermClick(idx)} sx={{ ...cardSx(selectedTerm === idx ? P.green : matchedTerms.has(idx) ? P.teal : P.blue), p: 1.5, cursor: 'pointer', textAlign: 'center', '&:hover': { transform: 'translate(-1px,-1px)', boxShadow: `5px 5px 0 ${P.blue.shadow}` }, transition: 'all 0.15s ease' }}>
                ...
                <Box sx={{ ...cardSx(P.yellow), p: 2 }}>
                  ...
                  <Stack spacing={1.5}>
                    {shuffledDefs.map((item, dIdx) => (
                      <Box key={dIdx} onClick={() => handleDefClick(dIdx)} sx={{ ...cardSx(matchedDefIndices.has(dIdx) ? P.teal : P.yellow), p: 1.5, cursor: selectedTerm !== null ? 'pointer' : 'default', '&:hover': selectedTerm !== null ? { transform: 'translate(-1px,-1px)', boxShadow: `5px 5px 0 ${P.yellow.shadow}` } : {}, transition: 'all 0.15s ease' }}>
            ...
            <Box sx={{ ...cardSx(P.green), mb: 3 }}>
              ...
              <Stack spacing={1} sx={{ mb: 3 }}>
                {PAIRS.map((p, idx) => {
                  ...
                  return (
                    <Box key={idx} sx={{ ...cardSx(isCorrect ? P.green : P.red), p: 1.5 }}>
                      <Typography variant="body2">{isCorrect ? '✓' : '✗'} <strong>{p.term}</strong> → {p.definition}</Typography>
                    </Box>
                  )
                })}
              </Stack>
              <Box component="button" onClick={() => navigate('/phase6/subphase/1/step/3/remedial/b2/task/d')} sx={{ ...cardSx(P.orange), p: 1.5, cursor: 'pointer', fontWeight: 'bold', color: P.orange.border, '&:hover': { transform: 'translate(-2px,-2px)', boxShadow: `6px 6px 0 ${P.orange.shadow}` }, transition: 'all 0.15s ease' }}>
                Next: Task D →
              </Box>
            </Box>
          </motion.div>
