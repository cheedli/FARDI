...
export default function Phase6SP1Step2RemedialB1TaskB() {
  ...
  React.useEffect(() => { window.__remedialSkip = () => navigate('/phase6/subphase/1/step/2/remedial/b1/task/c') }, [])
  ...
  const handleSubmit = async () => {
    ...
    try { await phase6API.logRemedialActivity(2, 'B1', 'B', filled, GUIDED_QUESTIONS.length, 0, 1) } catch (e) { console.error(e) }
  }
      ...
      <Container maxWidth="md">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}>
          <Box sx={{ ...cardSx(P.orange), mb: 3 }}>
            ...
            <Typography variant="body1" color="text.secondary">Write 6 sentences for a report summary</Typography>
          </Box>
        </motion.div>
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}>
          <Box sx={{ ...cardSx(P.teal), mb: 3 }}>
            <CharacterMessage speaker="Ms. Mabrouki" message="Now write your own sentences! Answer each question with a simple sentence. Use the examples to help you. Aim to write 6 complete sentences." />
          </Box>
        </motion.div>
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
          <Box sx={{ ...cardSx(P.blue), mb: 3, p: 2 }}>
            <Typography variant="body2">Answer each guided question with one sentence. Use past tense when possible (e.g., "was", "came", "fixed").</Typography>
          </Box>
        </motion.div>
        ...
        {!submitted ? (
          ...
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}>
            <Box sx={{ ...cardSx(P.green), textAlign: 'center' }}>
              ...
              <Typography variant="body1" sx={{ mb: 2 }}>{score >= 5 ? 'Excellent! Your report summary looks great!' : 'Good effort! Keep practicing your report writing.'}</Typography>
              <Box component="button" onClick={() => navigate('/phase6/subphase/1/step/2/remedial/b1/task/c')} sx={{ ...cardSx(P.orange), p: 1.5, cursor: 'pointer', fontWeight: 'bold', color: P.orange.border, '&:hover': { transform: 'translate(-2px,-2px)', boxShadow: `6px 6px 0 ${P.orange.shadow}` }, transition: 'all 0.15s ease' }}>
                Next: Task C →
              </Box>
            </Box>
          </motion.div>
