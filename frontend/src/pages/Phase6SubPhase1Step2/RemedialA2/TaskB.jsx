...
export default function Phase6SP1Step2RemedialA2TaskB() {
  ...
  React.useEffect(() => { window.__remedialSkip = () => navigate('/phase6/subphase/1/step/2/remedial/a2/task/c') }, [])
  const theme = useTheme()
  const P = theme.palette.mode === 'dark' ? DARK : LIGHT
  const { saveResponse } = useProgressSave({ phase: 6, subphase: 1, step: 2, interaction: 2, context: 'remedial_a2' })
  ...
  const handleSubmit = async () => {
    ...
    try { await phase6API.logRemedialActivity(2, 'A2', 'B', correct, SENTENCES.length, 0, 1) } catch (e) { console.error(e) }
  }
      ...
      <Container maxWidth="md">
        ...
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}>
          <Box sx={{ ...cardSx(P.teal), mb: 3 }}>
            <CharacterMessage speaker="Ms. Mabrouki" message="Fill Frenzy! Choose the correct word from the list to complete each sentence. Use the word bank to help you." />
          </Box>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
          <Box sx={{ ...cardSx(P.green), mb: 3 }}>
            ...
            <Stack direction="row" spacing={1} flexWrap="wrap" gap={1}>
              {WORD_BANK.map(w => (
                <Box key={w} sx={{ bgcolor: P.green.border, color: 'white', px: 2, py: 0.5, borderRadius: '12px', fontWeight: 'bold', fontSize: '0.85rem' }}>{w}</Box>
              ))}
            </Stack>
          </Box>
        </motion.div>
          ...
          {SENTENCES.map((s, idx) => {
            ...
            return (
              <motion.div key={idx} initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 + idx * 0.03 }}>
                <Box sx={{
                  ...cardSx(submitted ? (isCorrect ? P.green : { bg: '#FEF2F2', border: '#EF4444', shadow: '#B91C1C' }) : P.blue),
                  p: 2, display: 'flex', alignItems: 'center', gap: 1.5, flexWrap: 'wrap'
                }}>
                  <Typography variant="body1" sx={{ fontWeight: 500 }}>{idx + 1}. {s.before}</Typography>
                  ...
                  {isWrong && <Typography variant="body2" color="error" sx={{ fontWeight: 'bold' }}>✗ Correct: {s.blank}</Typography>}
