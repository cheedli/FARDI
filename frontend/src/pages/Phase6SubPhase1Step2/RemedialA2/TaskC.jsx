...
export default function Phase6SP1Step2RemedialA2TaskC() {
  ...
  React.useEffect(() => { resolveSubphase1RemedialNextUrl(2, 'A2').then(url => { window.__remedialSkip = () => navigate(url) }) }, [])
        ...
        {!submitted ? (
          ...
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}>
            <Box sx={{ ...cardSx(P.green), textAlign: 'center' }}>
              ...
              <Box component="button" onClick={async () => navigate(await resolveSubphase1RemedialNextUrl(2, 'A2'))} sx={{ ...cardSx(P.orange), p: 1.5, cursor: 'pointer', fontWeight: 'bold', color: P.orange.border, '&:hover': { transform: 'translate(-2px,-2px)', boxShadow: `6px 6px 0 ${P.orange.shadow}` }, transition: 'all 0.15s ease' }}>
                Continue →
              </Box>
            </Box>
          </motion.div>
