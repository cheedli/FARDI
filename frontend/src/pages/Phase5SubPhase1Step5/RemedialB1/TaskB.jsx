...
export default function Phase5Step5RemedialB1TaskB() {
  ...
  const handleSubmit = async () => {
    ...
    try { await phase5API.logRemedialActivity(5, 'B1', 'B', correctCount, 8, 0) } catch (e) { console.error(e) }
  }

  const handleContinue = () => { navigate('/phase5/subphase/1/step/5/remedial/b1/task/c') }
        ...
        {!submitted ? (
          <>
            ...
            <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
              <Box sx={{ ...clay(P.blue), mb: 3 }}>
                <Typography variant="h6" gutterBottom sx={{ color: P.red.border }}>Faulty: "{FAULTY_SENTENCES[currentIndex].faulty}"</Typography>
