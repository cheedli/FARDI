...
const SENTENCES = ['Ms. Mabrouki: Booth duty?', 'You: [First] [please] welcome.', 'SKANDER: Next?', 'You: [Then] [guide] and be [careful].', 'You: [Next] [help] guests.']
const CORRECT_ANSWERS = { 'g_1_0': 'first', 'g_1_1': 'please', 'g_3_0': 'then', 'g_3_1': 'guide', 'g_3_2': 'careful', 'g_4_0': 'next', 'g_4_1': 'help' }
...
export default function Phase5SubPhase2Step4RemedialB2TaskA() {
  ...
  const shuffledWordBank = useMemo(() => [...WORD_BANK_ORIGINAL, 'help'].sort(() => Math.random() - 0.5), [])
  ...
  const handleSubmit = async () => {
    ...
    try { await phase5API.logRemedialActivity(4, 'B2', 'A', finalScore, 7, 2) } catch (error) { console.error('Failed to log task completion:', error) }
  }

  const handleContinue = () => navigate('/phase5/subphase/2/step/4/remedial/b2/task/b')
      ...
      <Container maxWidth="md">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}>
          <Box sx={{ bgcolor: P.orange.bg, border: `2px solid ${P.orange.border}`, borderRadius: '20px', boxShadow: `4px 4px 0 ${P.orange.shadow}`, p: 3, mb: 3 }}>
            ...
            <Typography variant="h6" gutterBottom sx={{ color: P.orange.shadow }}>Task A: Role-Play Saga</Typography>
