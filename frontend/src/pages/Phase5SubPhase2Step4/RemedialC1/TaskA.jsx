...
const SENTENCES = ['Opponent: Why politeness?', 'You: [Politeness] builds cooperation.', 'Opponent: Sequencing?', 'You: [Sequencing] ensures understanding.']
const CORRECT_ANSWERS = { 'g_1_0': 'Politeness', 'g_3_0': 'Sequencing' }

export default function Phase5SubPhase2Step4RemedialC1TaskA() {
  const navigate = useNavigate()
  const theme = useTheme()
  const P = theme.palette.mode === 'dark' ? DARK : LIGHT
  const { saveResponse } = useProgressSave({ phase: 5, subphase: 2, step: 4, interaction: 1, context: 'remedial_c1' })
  const [answers, setAnswers] = useState({})
  const [submitted, setSubmitted] = useState(false)
  const [score, setScore] = useState(0)

  const shuffledWordBank = useMemo(() => [...WORD_BANK_ORIGINAL].map(w => w.charAt(0).toUpperCase() + w.slice(1)).sort(() => Math.random() - 0.5), [])
  ...
  const handleSubmit = async () => {
    ...
    try { await phase5API.logRemedialActivity(4, 'C1', 'A', finalScore, 2, 2) } catch (error) { console.error('Failed to log task completion:', error) }
  }

  const handleContinue = () => navigate('/phase5/subphase/2/step/4/remedial/c1/task/b')
      ...
      <Container maxWidth="md">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}>
          <Box sx={{ bgcolor: P.orange.bg, border: `2px solid ${P.orange.border}`, borderRadius: '20px', boxShadow: `4px 4px 0 ${P.orange.shadow}`, p: 3, mb: 3 }}>
            ...
            <Typography variant="h6" gutterBottom sx={{ color: P.orange.shadow }}>Task A: Debate Simulation</Typography>
