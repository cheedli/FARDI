...
const SENTENCES = ['[Please] welcome.', '[First] check.', '[Then] guide.', 'Be [careful].', '[Welcome] guests.', '[Help] people.', 'Say [thank you].', '[Guide] to booth.']
const CORRECT_ANSWERS = { 'g_0_0': 'please', 'g_1_0': 'first', 'g_2_0': 'then', 'g_3_0': 'careful', 'g_4_0': 'welcome', 'g_5_0': 'help', 'g_6_0': 'thank you', 'g_7_0': 'guide' }

export default function Phase5SubPhase2Step4RemedialA2TaskB() {
  const navigate = useNavigate()
  const theme = useTheme()
  const P = theme.palette.mode === 'dark' ? DARK : LIGHT
  const { saveResponse } = useProgressSave({ phase: 5, subphase: 2, step: 4, interaction: 2, context: 'remedial_a2' })
  ...
  const handleSubmit = async () => {
    ...
    try { await phase5API.logRemedialActivity(4, 'A2', 'B', finalScore, 8, 2) } catch (error) { console.error('Failed to log task completion:', error) }
  }

  const handleContinue = () => navigate('/phase5/subphase/2/step/4/remedial/a2/task/c')
