...
export default function Phase5SubPhase2Step4RemedialA2TaskA() {
  ...
  const handleSubmit = async () => {
    let correctCount = 0
    MATCHING_PAIRS.forEach(pair => { if (matches[pair.word] === pair.definition) correctCount++ })
    setScore(correctCount)
    setSubmitted(true)
    sessionStorage.setItem('phase5_subphase2_step4_remedial_a2_taskA_score', correctCount.toString())
    try { await phase5API.logRemedialActivity(4, 'A2', 'A', correctCount, 8, 2) } catch (error) { console.error('Failed to log task completion:', error) }
  }

  const handleContinue = () => navigate('/phase5/subphase/2/step/4/remedial/a2/task/b')
      ...
      <Container maxWidth="md">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}>
          <Box sx={{ bgcolor: P.orange.bg, border: `2px solid ${P.orange.border}`, borderRadius: '20px', boxShadow: `4px 4px 0 ${P.orange.shadow}`, p: 3, mb: 3 }}>
            ...
            <Typography variant="h6" gutterBottom sx={{ color: P.orange.shadow }}>Task A: Term Treasure Hunt</Typography>
