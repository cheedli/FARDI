...
export default function Phase5Step2RemedialC1Results() {
  ...
  const [nextUrl, setNextUrl] = useState('/phase5/subphase/1/step/2/remedial/c1/task/a')
  ...
  const calculateFinalScore = async () => {
    ...
    try {
      ...
      setNextUrl(backendNextUrl || '/phase5/subphase/1/step/2/remedial/c1/task/a')
    } catch (e) {
      console.error(e)
    }
  }
      ...
      <Container maxWidth="md">
        ...
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
          <Box sx={{ ...cardSx(scores.passed ? P.green : P.orange), mb: 3, textAlign: 'center' }}>
            ...
            <Typography variant="h5" sx={{ mt: 2 }} color="text.secondary">Total Score: {scores.total} / 23</Typography>
            <Typography variant="body1" sx={{ mt: 1 }} color="text.secondary">Pass Threshold: 19/23</Typography>
          </Box>
        </motion.div>
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}>
          <Box sx={{ ...cardSx(P.yellow), mb: 3 }}>
            ...
            <Stack spacing={2} sx={{ mt: 2 }}>
              {[
                { label: 'Task A (Debate Dominion)', value: scores.taskA, max: 4 },
                { label: 'Task B (Analysis Odyssey)', value: scores.taskB, max: 8 },
                { label: 'Task C (Expert Quiz)', value: scores.taskC, max: 6 },
                { label: 'Task D (Critique Kahoot)', value: scores.taskD, max: 6 },
              ].map((item, idx) => (
        ...
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
          <Box sx={{ ...cardSx(scores.passed ? P.green : P.yellow), mb: 3 }}>
            <Typography variant="body1" sx={{ color: scores.passed ? P.green.border : P.yellow.border }}>
              {scores.passed ? 'You will proceed to the next step. Great work!' : `Redirecting to repeat C1 remedial in ${countdown} seconds...`}
            </Typography>
          </Box>
          <Box component="button" onClick={handleRedirect} sx={{ width: '100%', bgcolor: scores.passed ? P.green.bg : P.orange.bg, border: `2px solid ${scores.passed ? P.green.border : P.orange.border}`, borderRadius: '14px', boxShadow: `4px 4px 0 ${scores.passed ? P.green.shadow : P.orange.shadow}`, py: 1.5, cursor: 'pointer', fontWeight: 'bold', fontSize: '1rem', color: scores.passed ? P.green.border : P.orange.border, '&:hover': { transform: 'translate(-2px,-2px)', boxShadow: `6px 6px 0 ${scores.passed ? P.green.shadow : P.orange.shadow}` }, transition: 'all 0.15s ease' }}>
            {scores.passed ? 'Continue to Next Step' : 'Retry C1 Remedial'}
          </Box>
        </motion.div>
      </Container>
    </Box>
