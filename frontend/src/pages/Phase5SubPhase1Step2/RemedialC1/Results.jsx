...
export default function Phase5Step2RemedialC1Results() {
  ...
  const calculateFinalScore = async () => {
    ...
    setLoading(false)
    try { await phase5API.logRemedialActivity(2, 'C1', 'final', total, { task_a_score: taskAScore, task_b_score: taskBScore, task_c_score: taskCScore, task_d_score: taskDScore }) } catch (e) { console.error(e) }
  }

  const handleRedirect = () => navigate(scores.passed ? '/dashboard' : '/phase5/step2/remedial-c1/task-a')
        ...
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
          <Box sx={{ ...cardSx(scores.passed ? P.green : P.yellow), mb: 3 }}>
            <Typography variant="body1" sx={{ color: scores.passed ? P.green.border : P.yellow.border }}>
              {scores.passed ? 'You will proceed to the dashboard. Great work!' : `Redirecting to repeat C1 remedial in ${countdown} seconds...`}
            </Typography>
          </Box>
