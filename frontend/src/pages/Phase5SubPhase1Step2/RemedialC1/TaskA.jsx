...
const CORRECT_ANSWERS = { 'g_1_0': 'backup', 'g_3_0': 'announce', 'g_3_1': 'update' }

export default function Phase5Step2RemedialC1TaskA() {
  const navigate = useNavigate()
  React.useEffect(() => { window.__remedialSkip = () => navigate('/phase5/subphase/1/step/2/remedial/c1/task/b') }, [])
  const theme = useTheme()
  const P = theme.palette.mode === 'dark' ? DARK : LIGHT
  const { saveResponse } = useProgressSave({ phase: 5, subphase: 1, step: 2, interaction: 1, context: 'remedial_c1' })
  ...
  const handleSubmit = async () => {
    let count = 0
    Object.entries(CORRECT_ANSWERS).forEach(([key, correct]) => { if (answers[key]?.toLowerCase().trim() === correct.toLowerCase()) count++ })
    setScore(count)
    setSubmitted(true)
    sessionStorage.setItem('phase5_step2_remedial_c1_taskA_score', count.toString())
    try { await phase5API.logRemedialActivity(2, 'C1', 'A', count, 3, 0) } catch (e) { console.error(e) }
  }
      ...
      <Container maxWidth="md">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}>
          <Box sx={{ ...cardSx(P.orange), mb: 3 }}>
            ...
            <Typography variant="body1" color="text.secondary">Simulate dialogue on crisis strategies</Typography>
          </Box>
        </motion.div>
        ...
        {submitted && (
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}>
            <Box sx={{ ...cardSx(P.green), mb: 3, textAlign: 'center' }}>
              ...
              <Typography variant="h6" sx={{ mt: 2 }} color="text.secondary">Score: {score} / 3</Typography>
            </Box>
            <Box component="button" onClick={() => navigate('/phase5/subphase/1/step/2/remedial/c1/task/b')} sx={{ width: '100%', bgcolor: P.green.bg, border: `2px solid ${P.green.border}`, borderRadius: '14px', boxShadow: `4px 4px 0 ${P.green.shadow}`, py: 1.5, cursor: 'pointer', fontWeight: 'bold', fontSize: '1rem', color: P.green.border, '&:hover': { transform: 'translate(-2px,-2px)', boxShadow: `6px 6px 0 ${P.green.shadow}` }, transition: 'all 0.15s ease' }}>
              Next: Task B →
            </Box>
          </motion.div>
        )}
