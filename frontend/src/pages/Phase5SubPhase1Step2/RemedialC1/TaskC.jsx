...
import { Box, Container, Typography, Stack, Radio, RadioGroup, FormControlLabel, FormControl, TextField } from '@mui/material'
...
export default function Phase5Step2RemedialC1TaskC() {
  ...
  React.useEffect(() => { window.__remedialSkip = () => navigate('/phase5/subphase/1/step/2/remedial/c1/task/d') }, [])
  ...
  const handleAnswerChange = (qId, value) => setAnswers(prev => ({ ...prev, [qId]: parseInt(value) }))
  const handleJustificationChange = (qId, value) => setJustifications(prev => ({ ...prev, [qId]: value }))

  const handleSubmit = async () => {
    let count = 0
    QUESTIONS.forEach(q => { if (answers[q.id] === q.correct && justifications[q.id]?.trim().length >= 5) count++ })
    setScore(count)
    setSubmitted(true)
    sessionStorage.setItem('phase5_step2_remedial_c1_taskC_score', count.toString())
    try { await phase5API.logRemedialActivity(2, 'C1', 'C', count, 6, 0) } catch (e) { console.error(e) }
  }
      ...
      <Container maxWidth="md">
        ...
        {!submitted ? (
          ...
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <Box component="button" onClick={() => navigate('/phase5/subphase/1/step/2/remedial/c1/task/d')} sx={{ width: '100%', bgcolor: P.green.bg, border: `2px solid ${P.green.border}`, borderRadius: '14px', boxShadow: `4px 4px 0 ${P.green.shadow}`, py: 1.5, cursor: 'pointer', fontWeight: 'bold', fontSize: '1rem', color: P.green.border, '&:hover': { transform: 'translate(-2px,-2px)', boxShadow: `6px 6px 0 ${P.green.shadow}` }, transition: 'all 0.15s ease' }}>
              Next: Task D →
            </Box>
          </motion.div>
        )}
