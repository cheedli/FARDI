...
export default function Phase5Step2RemedialB2TaskB() {
  ...
  React.useEffect(() => { window.__remedialSkip = () => navigate('/phase5/subphase/1/step/2/remedial/b2/task/c') }, [])
  ...
  const handleSubmit = async () => {
    ...
    answers.forEach(answer => {
      if (answer.trim().split(/\s+/).filter(w => w.length > 0).length >= 8) correctCount++
    })
    setScore(correctCount)
    setSubmitted(true)
    sessionStorage.setItem('phase5_step2_remedial_b2_taskB_score', correctCount.toString())
    try { await phase5API.logRemedialActivity(2, 'B2', 'B', correctCount, 8, 0) } catch (e) { console.error(e) }
  }
      ...
      <Container maxWidth="md">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}>
          <Box sx={{ ...cardSx(P.orange), mb: 3 }}>
            ...
            <Typography variant="h6" gutterBottom sx={{ color: P.orange.border }}>Task B: Crisis Response Plan</Typography>
            <Typography variant="body1" color="text.secondary">Write 8-sentence crisis response plan</Typography>
          </Box>
        </motion.div>
        ...
        {!submitted ? (
          ...
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <Box component="button" onClick={() => navigate('/phase5/subphase/1/step/2/remedial/b2/task/c')} sx={{ width: '100%', bgcolor: P.green.bg, border: `2px solid ${P.green.border}`, borderRadius: '14px', boxShadow: `4px 4px 0 ${P.green.shadow}`, py: 1.5, cursor: 'pointer', fontWeight: 'bold', fontSize: '1rem', color: P.green.border, '&:hover': { transform: 'translate(-2px,-2px)', boxShadow: `6px 6px 0 ${P.green.shadow}` }, transition: 'all 0.15s ease' }}>
              Next: Task C →
            </Box>
          </motion.div>
        )}
