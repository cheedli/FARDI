...
export default function Phase5Step2RemedialB2TaskD() {
  ...
  const handleSpellingChange = (value) => {
    const n = [...spellings]; n[currentIndex] = value; setSpellings(n)
  }
  const handleExplanationChange = (value) => {
    const n = [...explanations]; n[currentIndex] = value; setExplanations(n)
  }
  ...
  const handleSubmit = async () => {
    ...
    try { await phase5API.logRemedialActivity(2, 'B2', 'D', correctCount, 6, 0) } catch (e) { console.error(e) }
  }
  ...
  const handleContinue = async () => {
    ...
    const passed = totalScore >= Math.ceil((4 + 8 + 8 + 6) * 0.8)
    try { await phase5API.calculateRemedialScore(2, 'B2', { task_a_score: taskAScore, task_b_score: taskBScore, task_c_score: taskCScore, task_d_score: taskDScore }) } catch (e) { console.error(e) }
    navigate(passed ? '/dashboard' : '/phase5/subphase/1/step/2/remedial/b2/task/a')
  }

  const canSubmit = spellings[currentIndex].trim() && explanations[currentIndex].trim()
        ...
        {!submitted ? (
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}>
            <Box sx={{ ...cardSx(P.blue), mb: 3 }}>
              <Typography variant="h6" gutterBottom fontWeight="bold" sx={{ color: P.blue.border }}>Term {currentIndex + 1} of 6</Typography>
              ...
              <Box sx={{ display: 'flex', gap: 2 }}>
                ...
                {currentIndex === 5 ? (
                  <Box component="button" onClick={handleSubmit} disabled={!canSubmit} sx={{ flex: 2, bgcolor: P.orange.bg, border: `2px solid ${P.orange.border}`, borderRadius: '14px', boxShadow: `4px 4px 0 ${P.orange.shadow}`, py: 1.5, cursor: !canSubmit ? 'not-allowed' : 'pointer', fontWeight: 'bold', fontSize: '0.9rem', color: P.orange.border, opacity: !canSubmit ? 0.5 : 1, '&:hover': canSubmit ? { transform: 'translate(-2px,-2px)', boxShadow: `6px 6px 0 ${P.orange.shadow}` } : {}, transition: 'all 0.15s ease' }}>Submit All</Box>
                ) : (
                  <Box component="button" onClick={handleNext} disabled={!canSubmit} sx={{ flex: 2, bgcolor: P.blue.bg, border: `2px solid ${P.blue.border}`, borderRadius: '14px', boxShadow: `4px 4px 0 ${P.blue.shadow}`, py: 1.5, cursor: !canSubmit ? 'not-allowed' : 'pointer', fontWeight: 'bold', fontSize: '0.9rem', color: P.blue.border, opacity: !canSubmit ? 0.5 : 1, '&:hover': canSubmit ? { transform: 'translate(-2px,-2px)', boxShadow: `6px 6px 0 ${P.blue.shadow}` } : {}, transition: 'all 0.15s ease' }}>Next →</Box>
                )}
              </Box>
            </Box>
          </motion.div>
