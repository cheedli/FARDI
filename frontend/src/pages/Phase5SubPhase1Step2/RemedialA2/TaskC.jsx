...
export default function Phase5Step2RemedialA2TaskC() {
  ...
  React.useEffect(() => { window.__remedialSkip = () => navigate('/phase5/subphase/1/step/2/remedial/a2/task/a') }, [])
  ...
  const handleContinue = async () => {
    ...
    let nextUrl = '/phase5/subphase/1/step/2/remedial/a2/task/a'
    try {
      const result = await phase5API.calculateRemedialScore(2, 'A2', {
      ...
      <Container maxWidth="md">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}>
          <Box sx={{ ...cardSx(P.orange), mb: 3 }}>
            ...
            <Typography variant="body1" color="text.secondary">Write 6 simple sentences about the crisis</Typography>
          </Box>
        </motion.div>
        ...
        {!submitted ? (
            ...
            <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
              <Box sx={{ ...cardSx(P.blue), mb: 3 }}>
                <Typography variant="h6" gutterBottom fontWeight="bold" sx={{ color: P.blue.border }}>
                  Term: <strong>{SENTENCE_PROMPTS[currentIndex].term}</strong>
                </Typography>
                <Box sx={{ ...cardSx(P.teal), mb: 2 }}>
                  <Box sx={{ display: 'flex', alignItems: 'center' }}>
                    ...
                    <Typography variant="body2" sx={{ color: P.teal.border }}>
                      <strong>Hint:</strong> {SENTENCE_PROMPTS[currentIndex].hint}
                    </Typography>
                  </Box>
                </Box>
                <TextField
                  ...
                  rows={2}
                  variant="outlined"
                  placeholder={`Write a simple sentence using "${SENTENCE_PROMPTS[currentIndex].term}"...`}
                  ...
                  {currentIndex === 5 ? (
                    <Box
                      ...
                      sx={{
                        ...
                        '&:hover': userAnswers[currentIndex].trim() ? { transform: 'translate(-2px,-2px)', boxShadow: `6px 6px 0 ${P.orange.shadow}` } : {},
                        transition: 'all 0.15s ease',
                      }}
                    >
                    ...
                    <Box
                      ...
                      sx={{
                        ...
                        color: P.blue.border,
                        opacity: !userAnswers[currentIndex].trim() ? 0.5 : 1,
                        '&:hover': userAnswers[currentIndex].trim() ? { transform: 'translate(-2px,-2px)', boxShadow: `6px 6px 0 ${P.blue.shadow}` } : {},
                        transition: 'all 0.15s ease',
                      }}
                    >
                      Next →
          ...
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}>
            <Box sx={{ ...cardSx(P.yellow), mb: 3 }}>
              <Typography variant="h5" gutterBottom fontWeight="bold" sx={{ color: P.yellow.border }}>
                Results: {score} / 6 Correct
              </Typography>
