...
export default function Phase5Step2RemedialA2Results() {
  ...
  const [nextUrl, setNextUrl] = useState('/phase5/subphase/1/step/2/remedial/a2/task/a')
  ...
  const calculateFinalScore = async () => {
    ...
    try {
      ...
      setNextUrl(backendNextUrl || '/phase5/subphase/1/step/2/remedial/a2/task/a')
    } catch (error) {
      ...
      <Container maxWidth="md">
        ...
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
          <Box sx={{ ...cardSx(scores.passed ? P.green : P.orange), mb: 3, textAlign: 'center' }}>
            ...
            <Typography variant="h5" sx={{ mt: 2 }} color="text.secondary">Total Score: {scores.total} / 18</Typography>
            <Typography variant="body1" sx={{ mt: 1 }} color="text.secondary">Pass Threshold: 15/18</Typography>
          </Box>
        </motion.div>
          ...
          <Box sx={{ ...cardSx(P.yellow), mb: 3 }}>
            ...
            <Stack spacing={2} sx={{ mt: 2 }}>
              <Box sx={{ ...cardSx(P.blue), p: 2 }}>
                <Typography variant="body1" gutterBottom>Task A (Match Race): {scores.taskA} / 6</Typography>
                <LinearProgress variant="determinate" value={(scores.taskA / 6) * 100} sx={{ height: 8, borderRadius: 4 }} />
              </Box>
              <Box sx={{ ...cardSx(P.blue), p: 2 }}>
                <Typography variant="body1" gutterBottom>Task B (Fill Frenzy): {scores.taskB} / 6</Typography>
                <LinearProgress variant="determinate" value={(scores.taskB / 6) * 100} sx={{ height: 8, borderRadius: 4 }} />
              </Box>
              <Box sx={{ ...cardSx(P.blue), p: 2 }}>
                <Typography variant="body1" gutterBottom>Task C (Sentence Builder): {scores.taskC} / 6</Typography>
