...
export default function Phase5Step2RemedialB1TaskB() {
  ...
  React.useEffect(() => { window.__remedialSkip = () => navigate('/phase5/subphase/1/step/2/remedial/b1/task/c') }, [])
      ...
      <Container maxWidth="md">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}>
          <Box sx={{ ...cardSx(P.orange), mb: 3 }}>
            ...
            <Typography variant="h6" gutterBottom sx={{ color: P.orange.border }}>Task B: Proposal Builder</Typography>
            <Typography variant="body1" color="text.secondary">Write 6 sentences proposing crisis solutions</Typography>
          </Box>
        </motion.div>
            ...
            <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
              <Box sx={{ ...cardSx(P.blue), mb: 3 }}>
                <Typography variant="h6" gutterBottom fontWeight="bold" sx={{ color: P.blue.border }}>
                  {GUIDED_QUESTIONS[currentIndex].question}
                </Typography>
                <Box sx={{ ...cardSx(P.teal), mb: 2 }}>
                  <Box sx={{ display: 'flex', alignItems: 'center' }}>
                    ...
                    <Typography variant="body2" sx={{ color: P.teal.border }}>
                      <strong>Example:</strong> {GUIDED_QUESTIONS[currentIndex].example}
                    </Typography>
                  </Box>
                </Box>
                <TextField
                  ...
                  {currentIndex === 5 ? (
                    <Box
                      ...
                      sx={{
                        ...
                        py: 1.5,
                        cursor: !answers[currentIndex].trim() ? 'not-allowed' : 'pointer',
                        fontWeight: 'bold',
                        fontSize: '0.9rem',
                        color: P.orange.border,
                        opacity: !answers[currentIndex].trim() ? 0.5 : 1,
                        '&:hover': answers[currentIndex].trim() ? { transform: 'translate(-2px,-2px)', boxShadow: `6px 6px 0 ${P.orange.shadow}` } : {},
                        transition: 'all 0.15s ease',
                      }}
                    >
                      Submit All
                    ...
                    <Box
                      ...
                      sx={{
                        ...
                        color: P.blue.border,
                        opacity: !answers[currentIndex].trim() ? 0.5 : 1,
                        '&:hover': answers[currentIndex].trim() ? { transform: 'translate(-2px,-2px)', boxShadow: `6px 6px 0 ${P.blue.shadow}` } : {},
                        transition: 'all 0.15s ease',
                      }}
                    >
                      Next →
