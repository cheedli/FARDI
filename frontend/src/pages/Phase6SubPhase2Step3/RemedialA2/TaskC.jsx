...
export default function Phase6SP2Step3RemA2TaskC() {
  ...
  React.useEffect(() => { resolveSubphase2RemedialNextUrl(3, 'A2').then(url => { window.__remedialSkip = () => navigate(url) }) }, [])
      ...
      <Container maxWidth="md">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}>
          <Box sx={{ ...cardSx('orange'), mb: 3 }}>
            <Typography variant="h5" sx={{ fontWeight: 800, color: P.orange.border, mb: 0.5 }}>Step 3: Remedial A2 - Task C</Typography>
            <Typography>Sentence Builder: Choose the Correct Feedback Sentence</Typography>
          </Box>

          <Box sx={{ ...cardSx('blue'), mb: 3 }}>
            <Typography variant="body2">Choose the correct simple sentence about peer feedback words.</Typography>
          </Box>
          ...
          {!submitted ? (
            ...
            <Box sx={{ ...cardSx('green'), textAlign: 'center' }}>
              ...
              <Box
                component="button"
                onClick={async () => navigate(await resolveSubphase2RemedialNextUrl(3, 'A2'))}
