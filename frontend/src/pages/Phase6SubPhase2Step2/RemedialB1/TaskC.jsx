...
export default function Phase6SP2Step2RemB1TaskC() {
  ...
  React.useEffect(() => { resolveSubphase2RemedialNextUrl(2, 'B1').then(url => { window.__remedialSkip = () => navigate(url) }) }, [])
  const theme = useTheme()
  const P = theme.palette.mode === 'dark' ? DARK : LIGHT
  const { saveResponse } = useProgressSave({ phase: 6, subphase: 2, step: 2, interaction: 3, context: 'remedial_b1' })
      ...
      <Container maxWidth="md">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
          <Box sx={{ ...cardSx(P.orange), mb: 3 }}>
            <Typography variant="h5" fontWeight="bold" gutterBottom sx={{ color: P.orange.border }}>Step 2: Remedial B1 — Task C</Typography>
            <Typography variant="body1" sx={{ color: 'text.secondary' }}>Quiz Game: Identify Correct Feedback Approach</Typography>
          </Box>
        </motion.div>
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1, duration: 0.4 }}>
          <Alert severity="info" sx={{ mb: 3, borderRadius: '12px' }}>Answer 6 questions about feedback terms and what they mean.</Alert>
          {QUESTIONS.map((q, idx) => (
          ...
          {!submitted ? (
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}>
              <Box sx={{ ...cardSx(P.green), textAlign: 'center', mt: 2 }}>
                ...
                <Box component="button" onClick={async () => navigate(await resolveSubphase2RemedialNextUrl(2, 'B1'))}
