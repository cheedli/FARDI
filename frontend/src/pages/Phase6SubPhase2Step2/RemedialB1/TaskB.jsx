...
export default function Phase6SP2Step2RemB1TaskB() {
  ...
  React.useEffect(() => { window.__remedialSkip = () => navigate('/phase6/subphase/2/step/2/remedial/b1/task/c') }, [])
      ...
      <Container maxWidth="md">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
          <Box sx={{ ...cardSx(P.orange), mb: 3 }}>
            <Typography variant="h5" fontWeight="bold" gutterBottom sx={{ color: P.orange.border }}>Step 2: Remedial B1 — Task B</Typography>
            <Typography variant="body1" sx={{ color: 'text.secondary' }}>Writing Proposals: Write Trial Feedback Sentences</Typography>
          </Box>
        </motion.div>
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1, duration: 0.4 }}>
          <Alert severity="info" sx={{ mb: 3, borderRadius: '12px' }}>Write 6 polite trial feedback sentences. Use the positive sandwich: start positive, give a suggestion, end positively.</Alert>
          <Stack spacing={2}>
            {PROMPTS.map((p, idx) => (
              <motion.div key={idx} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 + idx * 0.05 }}>
                <Box sx={{ ...cardSx(P.blue) }}>
                  <Typography variant="subtitle1" fontWeight="bold" sx={{ mb: 0.5, color: P.blue.border }}>{idx + 1}. {p.label}</Typography>
                  <Typography variant="caption" color="text.secondary" sx={{ mb: 1, display: 'block' }}>Example: <em>{p.example}</em></Typography>
                  <TextField fullWidth multiline rows={2} value={answers[idx]} onChange={(e) => { const a = [...answers]; a[idx] = e.target.value; setAnswers(a) }} disabled={submitted} placeholder="Write your polite feedback sentence..." />
                </Box>
              </motion.div>
            ))}
          </Stack>
          {!submitted ? (
            <Box sx={{ textAlign: 'center', mt: 3 }}>
              <Box component="button" onClick={handleSubmit} disabled={!canSubmit}
                sx={{ px: 5, py: 1.5, borderRadius: '16px', border: `2px solid ${P.orange.border}`, bgcolor: P.orange.bg, boxShadow: `4px 4px 0 ${P.orange.shadow}`, fontWeight: 'bold', fontSize: '1rem', fontFamily: 'inherit', width: '100%', cursor: !canSubmit ? 'not-allowed' : 'pointer', opacity: !canSubmit ? 0.5 : 1, '&:hover:not(:disabled)': { transform: 'translate(-2px,-2px)', boxShadow: `6px 6px 0 ${P.orange.shadow}` }, transition: 'all 0.15s' }}>
                Submit
              </Box>
            </Box>
          ) : (
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}>
              <Box sx={{ ...cardSx(P.green), textAlign: 'center', mt: 3 }}>
                ...
                <Typography variant="h5" fontWeight="bold" sx={{ color: P.green.shadow }}>Task B Complete! Score: {score}/6</Typography>
                <Typography variant="body1" sx={{ mt: 1, color: 'text.secondary' }}>{score >= 5 ? 'Great polite feedback!' : 'Good effort! Remember to be polite and specific.'}</Typography>
                <Box component="button" onClick={() => navigate('/phase6/subphase/2/step/2/remedial/b1/task/c')}
