...
export default function Phase6SP2Step2RemB2TaskB() {
  ...
  React.useEffect(() => { window.__remedialSkip = () => navigate('/phase6/subphase/2/step/2/remedial/b2/task/c') }, [])
      ...
      <Container maxWidth="md">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
          <Box sx={{ ...cardSx(P.orange), mb: 3 }}>
            <Typography variant="h5" fontWeight="bold" gutterBottom sx={{ color: P.orange.border }}>Step 2: Remedial B2 — Task B</Typography>
            <Typography variant="body1" sx={{ color: 'text.secondary' }}>Writing: 8-Sentence Trial Peer Feedback Paragraph</Typography>
          </Box>
        </motion.div>
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1, duration: 0.4 }}>
          <Alert severity="info" sx={{ mb: 3, borderRadius: '12px' }}>Write 8 balanced, polite trial peer feedback sentences using the positive sandwich structure. Be specific and constructive.</Alert>
          <Stack spacing={2}>
            {PROMPTS.map((p, idx) => (
              <motion.div key={idx} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 + idx * 0.04 }}>
                <Box sx={{ ...cardSx(P.blue) }}>
                  ...
                  <TextField fullWidth multiline rows={2} value={answers[idx]} onChange={(e) => { const a = [...answers]; a[idx] = e.target.value; setAnswers(a) }} disabled={submitted} placeholder="Write your sentence here..." />
                </Box>
              </motion.div>
            ))}
          </Stack>
          {!submitted ? (
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}>
              <Box sx={{ ...cardSx(P.green), textAlign: 'center', mt: 3 }}>
                ...
                <Box component="button" onClick={() => navigate('/phase6/subphase/2/step/2/remedial/b2/task/c')}
