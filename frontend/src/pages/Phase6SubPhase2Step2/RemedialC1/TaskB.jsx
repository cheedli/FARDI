...
const DARK = { pageBg: '#0F0F1A', orange: { bg: '#431407', border: '#FB923C', shadow: '#9A3412' }, green: { bg: '#052E16', border: '#4ADE80', shadow: '#166534' }, blue: { bg: '#1E3A5F', border: '#60A5FA', shadow: '#1E40AF' } }
...
export default function Phase6SP2Step2RemC1TaskB() {
  ...
  React.useEffect(() => { window.__remedialSkip = () => navigate('/phase6/subphase/2/step/2/remedial/c1/task/c') }, [])
      ...
      <Container maxWidth="md">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
          <Box sx={{ ...cardSx(P.orange), mb: 3 }}>
            <Typography variant="h5" fontWeight="bold" gutterBottom sx={{ color: P.orange.border }}>Step 2: Remedial C1 — Task B</Typography>
            <Typography variant="body1" sx={{ color: 'text.secondary' }}>Writing: Sophisticated Trial Peer Feedback Analysis</Typography>
          </Box>
        </motion.div>
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1, duration: 0.4 }}>
          <Alert severity="info" sx={{ mb: 3, borderRadius: '12px' }}>Write 8 sophisticated, nuanced trial peer feedback sentences. Use: constructive, specific, balanced, empathy, actionable. Employ the positive sandwich and show genuine understanding of the writer's effort.</Alert>
          <Stack spacing={2}>
            {PROMPTS.map((p, idx) => (
              <motion.div key={idx} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 + idx * 0.04 }}>
                <Box sx={{ ...cardSx(P.blue) }}>
                  ...
                  <TextField fullWidth multiline rows={2} value={answers[idx]} onChange={(e) => { const a = [...answers]; a[idx] = e.target.value; setAnswers(a) }} disabled={submitted} placeholder="Write your sophisticated sentence here..." />
                </Box>
              </motion.div>
            ))}
          </Stack>
          {!submitted ? (
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}>
              <Box sx={{ ...cardSx(P.green), textAlign: 'center', mt: 3 }}>
                ...
                <Box component="button" onClick={() => navigate('/phase6/subphase/2/step/2/remedial/c1/task/c')}
