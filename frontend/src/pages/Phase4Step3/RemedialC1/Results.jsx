...
/**
 * Phase 4 Step 3 - Remedial C1 - Results Page
...
export default function RemedialC1Results() {
  ...
  useEffect(() => {
    ...
    fetch('/api/phase4/step4/remedial/c1/final-score', {
      ...
      .then(data => {
        if (data.success) {
          sessionStorage.setItem('phase4_step4_c1_next_url', data.data.next_url || (passed ? '/phase4/step/3' : '/phase4/step/3/remedial/c1/taskA'))
        }
      })
    ...
    const timer = setInterval(() => {
      setCountdown(prev => {
        if (prev <= 1) {
          ...
          navigate(sessionStorage.getItem('phase4_step4_c1_next_url') || (passed ? '/phase4/step/3' : '/phase4/step/3/remedial/c1/taskA'))
          return 0
        }
        return prev - 1
      })
