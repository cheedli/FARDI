...
export default function RemedialB2Results() {
  ...
  useEffect(() => {
    ...
    fetch('/api/phase4/step4/remedial/b2/final-score', {
      ...
      .then(data => {
        if (data.success) {
          sessionStorage.setItem('phase4_step4_b2_next_url', data.data.next_url || (passed ? '/phase4/step/3' : '/phase4/step/3/remedial/b2/taskA'))
        }
      })
    ...
    const timer = setInterval(() => {
      setCountdown(prev => {
        if (prev <= 1) {
          ...
          navigate(sessionStorage.getItem('phase4_step4_b2_next_url') || (passed ? '/phase4/step/3/remedial/c1/taskA' : '/phase4/step/3/remedial/b2/taskA'))
          return 0
        }
        return prev - 1
      })
