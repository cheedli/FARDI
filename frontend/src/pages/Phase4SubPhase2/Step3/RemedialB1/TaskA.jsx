...
export default function Phase4_2Step3RemedialB1TaskA() {
  ...
  React.useEffect(() => { window.__remedialSkip = () => navigate('/phase4_2/step/3/remedial/b1/taskB') }, [])
  const { saveResponse } = useProgressSave({ phase: 4, subphase: 2, step: 3, interaction: 1, context: 'remedial_b1' })
        ...
        <Container maxWidth="md">
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
            <Box sx={{ bgcolor: P.blue.bg, border: `2px solid ${P.blue.border}`, borderRadius: '20px', boxShadow: `4px 4px 0 ${P.blue.shadow}`, p: 3, mb: 3 }}>
              <Typography variant="h5" sx={{ fontWeight: 'bold', color: P.blue.shadow }}>Phase 4.2 Step 3 - Remedial B1</Typography>
              <Typography variant="h6" sx={{ color: P.blue.shadow }}>Task A: Role-Play Dialogue Complete!</Typography>
            </Box>
            <Box sx={{ bgcolor: P.green.bg, border: `2px solid ${P.green.border}`, borderRadius: '20px', boxShadow: `4px 4px 0 ${P.green.shadow}`, p: 4, mb: 3, textAlign: 'center' }}>
              <Typography variant="h2" sx={{ fontWeight: 'bold', color: P.green.border }}>{finalScore} / 10</Typography>
              ...
              <Typography variant="body2" sx={{ mt: 1, opacity: 0.6 }}>Raw score: {score} / 4 blanks</Typography>
            </Box>
            <Box sx={{ textAlign: 'center' }}>
              <Box component="button" onClick={() => navigate('/phase4_2/step/3/remedial/b1/taskB')} sx={{
                bgcolor: P.green.bg, border: `2px solid ${P.green.border}`,
              ...
              }}>Continue to Task B <ArrowForwardIcon /></Box>
            </Box>
          </motion.div>
        </Container>
      </Box>
    ...
    <Box sx={{ minHeight: '100vh', bgcolor: P.pageBg, py: 4 }}>
      <Container maxWidth="md">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
          ...
          <Box sx={{ bgcolor: P.blue.bg, border: `2px solid ${P.blue.border}`, borderRadius: '20px', boxShadow: `4px 4px 0 ${P.blue.shadow}`, p: 3, mb: 3 }}>
            <Typography variant="h5" sx={{ fontWeight: 'bold', color: P.blue.shadow }}>Phase 4.2 Step 3 - Remedial B1</Typography>
            <Typography variant="h6" sx={{ color: P.blue.shadow }}>Task A: Role-Play Dialogue</Typography>
            <Typography variant="body2" sx={{ mt: 0.5 }}>Complete the dialogue about planning a social media post!</Typography>
          </Box>
          ...
          {/* Chat window */}
          <Box sx={{
            bgcolor: isDark ? '#1a1a2e' : '#E5DDD5',
            border: `2px solid ${P.blue.border}`,
            borderRadius: '20px', boxShadow: `4px 4px 0 ${P.blue.shadow}`,
            overflow: 'hidden', mb: 3
          }}>
            {/* Chat header */}
            <Box sx={{ bgcolor: isDark ? '#075E54' : '#075E54', color: 'white', p: 1.5, display: 'flex', alignItems: 'center', gap: 2 }}>
              <Avatar sx={{ bgcolor: '#25D366', width: 36, height: 36 }}>
                <Typography variant="caption" fontWeight="bold">B1</Typography>
              </Avatar>
              <Box sx={{ flex: 1 }}>
              ...
              <Box component="span" sx={{
                bgcolor: 'rgba(255,255,255,0.2)', borderRadius: '999px', px: 1.5, py: 0.3,
                fontSize: '0.8rem', fontWeight: 700, color: 'white'
              }}>{Math.round((score / 4) * 10)}/10</Box>
            </Box>
            ...
            <Box sx={{ p: 2, minHeight: 400, maxHeight: 500, overflowY: 'auto', backgroundImage: isDark ? 'none' : 'repeating-linear-gradient(45deg, transparent, transparent 10px, rgba(0,0,0,.03) 10px, rgba(0,0,0,.03) 20px)' }}>
              {displayedMessages.map((message, index) => (
                <Box key={index} sx={{ display: 'flex', justifyContent: message.sender === 'YOU' ? 'flex-end' : 'flex-start', mb: 1.5, alignItems: 'flex-end' }}>
                  ...
                  <Box sx={{
                    maxWidth: '70%', p: 1.5,
                    bgcolor: message.sender === 'YOU' ? '#DCF8C6' : (isDark ? '#2d2d2d' : 'white'),
                    borderRadius: message.sender === 'YOU' ? '8px 8px 0 8px' : '8px 8px 8px 0',
                    border: `1px solid ${message.sender === 'YOU' ? '#b5e7a0' : (isDark ? '#444' : '#e0e0e0')}`
                  }}>
                    {message.sender !== 'YOU' && (
            ...
            <Box sx={{ bgcolor: isDark ? '#111' : '#F0F0F0', p: 2, borderTop: `1px solid ${isDark ? '#333' : '#D1D1D1'}` }}>
              ...
              {selectedWord && (
                <Box sx={{ bgcolor: P.yellow.bg, border: `2px solid ${P.yellow.border}`, borderRadius: '8px', p: 1, mt: 1.5 }}>
                  <Typography variant="caption" sx={{ fontWeight: 700, fontSize: '0.9rem', color: P.yellow.shadow }}>
