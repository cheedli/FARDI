...
import { Box, Typography, Avatar, LinearProgress, Container } from '@mui/material'
...
const WORD_BANK_ORIGINAL = ['hashtag', 'because', 'viral', 'caption', 'emoji', 'tag']
const AVATARS = {
...
export default function Phase4_2RemedialA2TaskA() {
  ...
  React.useEffect(() => { window.__remedialSkip = () => navigate('/phase4_2/step/1/remedial/a2/taskB') }, [])
    ...
    if (message.type === 'response') {
      if (message.sent) {
        ...
        return <Typography variant="body1" sx={{ color: '#2c3e50', fontWeight: 600 }}>{parts.map((part, idx) => (
          <React.Fragment key={idx}>{part}{idx < parts.length - 1 && message.blanks && (
        ...
        <Box>
              ...
              <React.Fragment key={idx}>
                ...
                {idx < parts.length - 1 && message.blanks && (
                  <Box component="span" onClick={() => handleBlankClick(message.id, idx)} sx={{
                    ...
                    borderRadius: '8px', px: 1.5, py: 0.5, fontWeight: 700, fontSize: '0.8rem',
          ...
          {allFilled && (
            <Box component="button" onClick={() => handleSendMessage(message.id)} sx={{
              ...
              px: 2, py: 0.5, fontWeight: 700, fontSize: '0.85rem', cursor: 'pointer', color: P.green.shadow,
        ...
        <Container maxWidth="md">
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
            ...
            <Box sx={{ bgcolor: P.green.bg, border: `2px solid ${P.green.border}`, borderRadius: '20px', boxShadow: `4px 4px 0 ${P.green.shadow}`, p: 4, textAlign: 'center', mb: 3 }}>
              <Typography variant="h3" gutterBottom fontWeight="bold" sx={{ color: P.green.shadow }}>{score} / 6</Typography>
              <Typography variant="h6" sx={{ color: P.green.shadow }}>Points Earned</Typography>
              <Typography variant="body1" sx={{ mt: 2, color: P.green.shadow }}>Adventure Progress: {((score / 6) * 100).toFixed(0)}% Complete</Typography>
            </Box>
            <Box sx={{ textAlign: 'center' }}>
              <Box component="button" onClick={() => navigate('/phase4_2/step/1/remedial/a2/taskB')} sx={{
                bgcolor: P.green.bg, border: `2px solid ${P.green.border}`, borderRadius: '12px', boxShadow: `3px 3px 0 ${P.green.shadow}`,
                px: 4, py: 1.5, fontWeight: 700, fontSize: '1.1rem', cursor: 'pointer', color: P.green.shadow,
      ...
      <Container maxWidth="md">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
          ...
          <Box sx={{ bgcolor: P.blue.bg, border: `2px solid ${P.blue.border}`, borderRadius: '20px', boxShadow: `4px 4px 0 ${P.blue.shadow}`, p: 3, mb: 3 }}>
            ...
            <Typography variant="body1" sx={{ color: P.blue.shadow }}>Complete the dialogue about social media!</Typography>
          </Box>
          ...
          {/* Chat Interface */}
          <Box sx={{ bgcolor: P.orange.bg, border: `2px solid ${P.orange.border}`, borderRadius: '20px', boxShadow: `4px 4px 0 ${P.orange.shadow}`, overflow: 'hidden', maxWidth: 600, mx: 'auto' }}>
            {/* Chat Header */}
            <Box sx={{ bgcolor: P.blue.bg, borderBottom: `2px solid ${P.blue.border}`, p: 1.5, display: 'flex', alignItems: 'center', gap: 2 }}>
              <Avatar sx={{ bgcolor: P.green.border, width: 36, height: 36 }}><Typography variant="caption" fontWeight="bold" sx={{ color: 'white' }}>A2</Typography></Avatar>
              <Box sx={{ flex: 1 }}>
                <Typography variant="subtitle2" fontWeight="bold" sx={{ color: P.blue.shadow }}>Social Media Team Chat</Typography>
                <Typography variant="caption" sx={{ fontSize: '0.7rem', color: P.blue.shadow }}>Ms. Mabrouki, Skander, You</Typography>
              </Box>
              <Box component="span" sx={{ bgcolor: P.yellow.bg, border: `2px solid ${P.yellow.border}`, borderRadius: '999px', px: 2, py: 0.5, fontSize: '0.85rem', fontWeight: 700, color: P.yellow.shadow }}>
                {score}/6
              </Box>
            </Box>
            ...
            <Box sx={{ p: 2, minHeight: 400, maxHeight: 500, overflowY: 'auto', bgcolor: isDark ? '#1a1a2e' : '#E5DDD5' }}>
              {displayedMessages.map((message, index) => (
                <Box key={index} sx={{ display: 'flex', justifyContent: message.sender === 'YOU' ? 'flex-end' : 'flex-start', mb: 1.5, alignItems: 'flex-end' }}>
                  ...
                  <Box sx={{ maxWidth: '70%', p: 1.5, bgcolor: message.sender === 'YOU' ? '#DCF8C6' : 'white', borderRadius: message.sender === 'YOU' ? '8px 8px 0 8px' : '8px 8px 8px 0', boxShadow: '1px 1px 3px rgba(0,0,0,0.1)' }}>
            ...
            {/* Word Bank */}
            <Box sx={{ bgcolor: P.yellow.bg, borderTop: `2px solid ${P.yellow.border}`, p: 2 }}>
              <Typography variant="caption" sx={{ display: 'block', mb: 1, color: P.yellow.shadow, fontWeight: 700, fontSize: '0.85rem' }}>Tap words to auto-fill blanks (tap filled blank to remove):</Typography>
              <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
                {wordBank.map((word, idx) => (
                  <Box key={idx} component="span" onClick={() => handleWordClick(word)} sx={{
                    bgcolor: selectedWord === word ? P.green.bg : 'white',
                    border: `2px solid ${selectedWord === word ? P.green.border : '#D1D1D1'}`,
                    borderRadius: '8px', px: 1.5, py: 0.5, fontWeight: selectedWord === word ? 'bold' : '600',
                    cursor: 'pointer', fontSize: '0.85rem', color: selectedWord === word ? P.green.shadow : '#2c3e50',
                    display: 'inline-block',
                    '&:hover': { bgcolor: selectedWord === word ? P.green.bg : '#E8E8E8' }
                  }}>{word}</Box>
                ))}
              </Box>
              {selectedWord && (
                <Box sx={{ mt: 1.5, bgcolor: P.yellow.bg, border: `2px solid ${P.yellow.border}`, borderRadius: '8px', p: 1 }}>
                  <Typography variant="caption" sx={{ color: P.yellow.shadow, fontWeight: 700 }}>Filling next blank with: "{selectedWord}"</Typography>
                </Box>
              )}
            </Box>
