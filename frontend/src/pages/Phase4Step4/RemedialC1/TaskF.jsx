...
/**
 * Phase 4 Step 4 - Remedial C1 - Task F: Debate Duel Advanced
...
const DEBATE_SENTENCES = [
  { id: 1, faulty: 'It is crucial that promotional advertising is balanced with ethical considerations so as not to alienate the audience.', correctAnswer: 'It is crucial that promotional advertising be balanced with ethical considerations so as not to alienate the audience.', errors: ['Subjunctive error: "is" should be "be" after "it is crucial that"'] },
  { id: 2, faulty: 'Persuasive techniques should incorporates pathos only when they are supported by credible ethos, as video 1 illustrates.', correctAnswer: 'Persuasive techniques should incorporate pathos only when they are supported by credible ethos, as video 1 illustrates.', errors: ['Modal error: "incorporates" should be "incorporate" (base form after modal "should")'] },
  { id: 3, faulty: 'If targeted strategies was applied without respect for privacy, they might erode consumer trust in the long term.', correctAnswer: 'If targeted strategies were applied without respect for privacy, they might erode consumer trust in the long term.', errors: ['Conditional error: "was" should be "were" in second conditional'] },
  { id: 4, faulty: 'It is essential that originality is prioritized in creative execution, lest advertisements become forgettable.', correctAnswer: 'It is essential that originality be prioritized in creative execution, lest advertisements become forgettable.', errors: ['Subjunctive error: "is prioritized" should be "be prioritized" after "it is essential that"'] },
  { id: 5, faulty: 'Dramatisation could captivates viewers more deeply if obstacles were portrayed as authentic rather than contrived.', correctAnswer: 'Dramatisation could captivate viewers more deeply if obstacles were portrayed as authentic rather than contrived.', errors: ['Modal error: "captivates" should be "captivate" (base form after modal "could")'] },
  { id: 6, faulty: 'Ethical advertising must remains a non-negotiable priority, even when commercial pressures threaten to compromise integrity.', correctAnswer: 'Ethical advertising must remain a non-negotiable priority, even when commercial pressures threaten to compromise integrity.', errors: ['Modal error: "remains" should be "remain" (base form after modal "must")'] }
]

export default function Phase4Step5RemedialC1TaskF() {
  const navigate = useNavigate()
  React.useEffect(() => { window.__remedialSkip = () => navigate('/phase4/step/4/remedial/c1/taskG') }, [])
  const theme = useTheme()
  const P = theme.palette.mode === 'dark' ? DARK : LIGHT
  const { saveResponse } = useProgressSave({ phase: 4, subphase: null, step: 5, interaction: 6, context: 'remedial_c1' })
  const [answers, setAnswers] = useState(() => {
    const initial = {}
    DEBATE_SENTENCES.forEach(s => { initial[s.id] = s.faulty })
    return initial
  })
  ...
  const logTaskCompletion = async (score) => {
    ...
    try {
      await fetch('/api/phase4/step5/remedial/log', { method: 'POST', headers: { 'Content-Type': 'application/json' }, credentials: 'include', body: JSON.stringify({ level: 'C1', task: 'F', score, max_score: 6, completed: true }) })
    } catch (error) { console.error('Failed to log task completion:', error) }
  }

  const allFilled = DEBATE_SENTENCES.every(s => (answers[s.id] && answers[s.id].trim()) || s.faulty)
      ...
      <Container maxWidth="md">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <Box sx={{ ...cardSx('orange'), mb: 3 }}>
            <Typography variant="h4" sx={{ fontWeight: 800, color: P.orange.border, mb: 1 }}>
              Phase 4 Step 4: Evaluate - Remedial Practice
            </Typography>
            <Typography variant="h5" sx={{ fontWeight: 700, mb: 1 }}>Level C1 - Task F: Debate Duel Advanced 🎭</Typography>
            <Typography>Fix 6 debate sentences with subjunctive/modal errors!</Typography>
          </Box>
          ...
          <Box sx={{ ...cardSx('purple'), mb: 3 }}>
            <Typography variant="h6" sx={{ fontWeight: 700, color: P.purple.border, mb: 2 }}>🎭 What to Fix (C1 Level):</Typography>
            <Stack spacing={1}>
              <Typography variant="body2"><strong>Present Subjunctive:</strong> Use base form after "it is crucial/essential that" (be, not is/are)</Typography>
              <Typography variant="body2"><strong>Modals + Base Form:</strong> should/could/might/must + BASE VERB (incorporate, not incorporates)</Typography>
              <Typography variant="body2"><strong>Second Conditional:</strong> Use "were" (not "was") in if-clauses (If X were..., Y might...)</Typography>
              <Typography variant="body2"><strong>Advanced Structures:</strong> "lest" + base form, implied subjunctive</Typography>
            </Stack>
          </Box>
          ...
          {submitted && (
            <Stack spacing={3}>
              ...
              <Box
                component="button"
                onClick={() => navigate('/phase4/step/4/remedial/c1/taskG')}
