...
export default function Phase6SP1Step4RemB1TaskC() {
  ...
  React.useEffect(() => { resolveSubphase1RemedialNextUrl(4, 'B1').then(url => { window.__remedialSkip = () => navigate(url) }) }, [])
        ...
        {!submitted ? (
          ...
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}>
            <Box sx={{ ...cardSx(P.purple), textAlign: 'center' }}>
              ...
              <Box
                component="button"
                onClick={async () => navigate(await resolveSubphase1RemedialNextUrl(4, 'B1'))}
                sx={{
                  px: 6, py: 1.5,
                  bgcolor: P.purple.bg,
