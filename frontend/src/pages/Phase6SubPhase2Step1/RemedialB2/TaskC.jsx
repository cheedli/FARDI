...
export default function Phase6SP2Step1RemB2TaskC() {
  ...
  React.useEffect(() => { window.__remedialSkip = () => navigate('/phase6/subphase/2/step/1/remedial/b2/task/d') }, [])
      ...
      <Container maxWidth="lg">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
          <Box sx={{ ...cardSx(P.orange), mb: 3 }}>
            <Typography variant="h5" fontWeight="bold" gutterBottom sx={{ color: P.orange.border }}>Step 1: Remedial B2 — Task C</Typography>
            <Typography variant="body1" sx={{ color: 'text.secondary' }}>Matching Game: Feedback Terms and Functions</Typography>
          </Box>
        </motion.div>
        ...
        {!submitted ? (
                    ...
                    {PAIRS.map((p, idx) => (
                      <Box
                        ...
                        sx={{
                          ...
                          borderColor: selectedTerm === idx ? P.purple.border : matchedTerms.has(idx) ? P.green.border : P.blue.border,
                          bgcolor: selectedTerm === idx ? P.purple.bg : matchedTerms.has(idx) ? P.green.bg : P.blue.bg,
                          cursor: 'pointer', textAlign: 'center', fontWeight: 'bold', fontFamily: 'inherit', fontSize: '1rem', width: '100%',
                          transition: 'all 0.15s', '&:hover': { transform: 'translate(-1px,-1px)' },
                        }}
                      >{p.term}</Box>
                    ))}
                  </Stack>
                  ...
                  <Stack spacing={1.5}>
                    {shuffledFunctions.map((item, fIdx) => (
                      <Box
                        key={fIdx}
                        component="button"
                        onClick={() => handleFuncClick(fIdx)}
                        sx={{
                          p: 1.5, borderRadius: '12px', border: '2px solid',
                          borderColor: matchedFuncs.has(fIdx) ? P.green.border : selectedTerm !== null ? P.purple.border : P.blue.border,
                          bgcolor: matchedFuncs.has(fIdx) ? P.green.bg : P.blue.bg,
                          cursor: selectedTerm !== null ? 'pointer' : 'default', textAlign: 'left', fontFamily: 'inherit', fontSize: '0.9rem', width: '100%',
            ...
            <Box sx={{ mt: 3, textAlign: 'center' }}>
              <Box
                ...
                sx={{
                  ...
                  cursor: matchedTerms.size < PAIRS.length ? 'not-allowed' : 'pointer',
                  opacity: matchedTerms.size < PAIRS.length ? 0.5 : 1,
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}>
            <Box sx={{ ...cardSx(P.green), textAlign: 'center' }}>
              ...
              <Box
                component="button"
                onClick={() => navigate('/phase6/subphase/2/step/1/remedial/b2/task/d')}
                sx={{
