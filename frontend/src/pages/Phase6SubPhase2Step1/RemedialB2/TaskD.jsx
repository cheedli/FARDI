...
export default function Phase6SP2Step1RemB2TaskD() {
  ...
  React.useEffect(() => { resolveSubphase2RemedialNextUrl(1, 'B2').then(url => { window.__remedialSkip = () => navigate(url) }) }, [])
  ...
  const handleNext = () => {
    ...
    const newResults = [...results, { spelling: spelling.trim(), explanation: explanation.trim(), correct: isCorrectSpell, hasExplanation, pts }]
        ...
        <Container maxWidth="md">
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}>
            <Box sx={{ ...cardSx(P.green), textAlign: 'center' }}>
              ...
              <Box
                component="button"
                onClick={async () => navigate(await resolveSubphase2RemedialNextUrl(1, 'B2'))}
                sx={{
      ...
      <Container maxWidth="md">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
          <Box sx={{ ...cardSx(P.orange), mb: 3 }}>
            <Typography variant="h5" fontWeight="bold" gutterBottom sx={{ color: P.orange.border }}>Step 1: Remedial B2 — Task D</Typography>
            <Typography variant="body1" sx={{ color: 'text.secondary' }}>Spell & Explain: Feedback Terms</Typography>
          </Box>
        </motion.div>

        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1, duration: 0.4 }}>
          <Alert severity="info" sx={{ mb: 3, borderRadius: '12px' }}>For each term: first spell it correctly, then explain what it means in your own words.</Alert>

          <LinearProgress
            variant="determinate"
            value={(current / TERMS.length) * 100}
            sx={{ mb: 1, height: 8, borderRadius: 4, bgcolor: P.orange.bg, '& .MuiLinearProgress-bar': { bgcolor: P.orange.border } }}
          />
          <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>Term {current + 1} of {TERMS.length}</Typography>

          <Box sx={{ ...cardSx(P.teal) }}>
            <Typography variant="h6" sx={{ mb: 2, color: P.teal.border }}>
              Term: <strong style={{ fontSize: '1.4rem' }}>{'_'.repeat(TERMS[current].term.length)}</strong>
            </Typography>
            <Alert severity="info" sx={{ mb: 3, borderRadius: '12px' }}>
              Hint: This word has {TERMS[current].term.length} letters and means: "{TERMS[current].meaning}"
            </Alert>
            <Stack spacing={3}>
              <Box>
                <Typography variant="subtitle1" fontWeight="bold" sx={{ mb: 1 }}>Spell the term:</Typography>
                <TextField
                  fullWidth value={spelling} onChange={(e) => setSpelling(e.target.value)}
                  placeholder="Type the spelling here..." variant="outlined"
                />
              </Box>
              <Box>
                <Typography variant="subtitle1" fontWeight="bold" sx={{ mb: 1 }}>Explain the meaning in your own words:</Typography>
                <TextField
                  fullWidth multiline rows={3} value={explanation} onChange={(e) => setExplanation(e.target.value)}
                  placeholder="Write what this word means..." variant="outlined"
                />
              </Box>
            </Stack>
            <Box sx={{ mt: 3 }}>
              <Box
                ...
                sx={{
                  ...
                  cursor: (!spelling.trim() || !explanation.trim()) ? 'not-allowed' : 'pointer',
                  opacity: (!spelling.trim() || !explanation.trim()) ? 0.5 : 1,
