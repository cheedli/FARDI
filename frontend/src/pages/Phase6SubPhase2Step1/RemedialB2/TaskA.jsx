...
export default function Phase6SP2Step1RemB2TaskA() {
  ...
  React.useEffect(() => { window.__remedialSkip = () => navigate('/phase6/subphase/2/step/1/remedial/b2/task/b') }, [])
      ...
      <Container maxWidth="md">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
          <Box sx={{ ...cardSx(P.orange), mb: 3 }}>
            <Typography variant="h5" fontWeight="bold" gutterBottom sx={{ color: P.orange.border }}>Step 1: Remedial B2 — Task A</Typography>
            <Typography variant="body1" sx={{ color: 'text.secondary' }}>Role-Play Dialogue: Giving Peer Feedback</Typography>
          </Box>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1, duration: 0.4 }}>
          ...
          <Alert severity="info" sx={{ mb: 3, borderRadius: '12px' }}>Complete the peer feedback dialogue by choosing the correct feedback words.</Alert>
          ...
          <Box sx={{ ...cardSx(submitted ? (ans2 === 'weakness' && ans3 === 'suggestion' ? P.green : P.red) : P.teal), mb: 3 }}>
            <Chip label="You" size="small" sx={{ bgcolor: P.teal.border, color: 'white', mb: 1 }} />
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, flexWrap: 'wrap' }}>
          ...
          {!submitted ? (
            <Box
              ...
              sx={{
                ...
                cursor: (!ans1 || !ans2 || !ans3) ? 'not-allowed' : 'pointer',
                opacity: (!ans1 || !ans2 || !ans3) ? 0.5 : 1,
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}>
              <Box sx={{ ...cardSx(P.green), textAlign: 'center' }}>
                ...
                <Box
                  component="button"
                  onClick={() => navigate('/phase6/subphase/2/step/1/remedial/b2/task/b')}
                  sx={{
