...
export default function Phase6SP2Step1RemC1TaskB() {
  ...
  React.useEffect(() => { window.__remedialSkip = () => navigate('/phase6/subphase/2/step/1/remedial/c1/task/c') }, [])
      ...
      <Container maxWidth="md">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
          <Box sx={{ ...cardSx(P.orange), mb: 3 }}>
            <Typography variant="h5" fontWeight="bold" gutterBottom sx={{ color: P.orange.border }}>Step 1: Remedial C1 — Task B</Typography>
            <Typography variant="body1" sx={{ color: 'text.secondary' }}>Writing: Sophisticated 8-Sentence Peer Feedback Analysis</Typography>
          </Box>
        </motion.div>

        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1, duration: 0.4 }}>
          <Alert severity="info" sx={{ mb: 3, borderRadius: '12px' }}>
            Write 8 sophisticated, empathetic sentences of peer feedback using advanced vocabulary: constructive, specific, balanced, empathy, actionable, growth mindset. Use the positive sandwich structure.
          </Alert>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}>
              <Box sx={{ ...cardSx(P.green), textAlign: 'center', mt: 3 }}>
                ...
                <Typography variant="body1" sx={{ mt: 1, color: 'text.secondary' }}>
                  {score >= 6 ? 'Excellent sophisticated feedback!' : 'Good effort! Try to use more advanced vocabulary and empathetic tone.'}
                </Typography>
                <Box
                  component="button"
                  onClick={() => navigate('/phase6/subphase/2/step/1/remedial/c1/task/c')}
                  sx={{
