...
export default function Phase4_2RemedialB1Results() {
  ...
  useEffect(() => {
    ...
    setResults({ taskA: taskAScore, taskB: taskBScore, taskC: taskCScore, taskD: taskDScore, taskE: taskEScore, taskF: taskFScore, total: averageScore, passed })
    logRemedialCompletion(averageScore, passed)
  }, [])

  const logRemedialCompletion = async (score) => {
    try {
      const data = await requestPhase42FinalScore(1, 'b1', { total_score: score })
  ...
  const handleContinue = () => {
    ...
    if (results.passed) navigate('/phase4_2/step/1/remedial/b2/taskA')
    else navigate('/phase4_2/step/1/remedial/b1/taskA')
  }
  ...
  const TASKS = [
    { label: 'Task A: Fill Quest', score: results.taskA },
    { label: 'Task B: Writing Proposals', score: results.taskB },
    { label: 'Task C: Quiz Game', score: results.taskC },
    { label: 'Task D: Tense Odyssey', score: results.taskD },
    { label: 'Task E: Tense Time Travel', score: results.taskE },
    { label: 'Task F: Subject-Verb Agreement', score: results.taskF },
  ]
      ...
      <Container maxWidth="md">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
          ...
          <Box sx={{ bgcolor: R.bg, border: `2px solid ${R.border}`, borderRadius: '20px', boxShadow: `4px 4px 0 ${R.shadow}`, p: 3, mb: 3 }}>
            <Typography variant="h5" gutterBottom fontWeight="bold" sx={{ color: R.shadow }}>Phase 4.2 Step 1 - Level B1 Remedial</Typography>
            <Typography variant="h6" sx={{ color: R.shadow }}>Final Results</Typography>
          </Box>

          <Box sx={{ bgcolor: P.teal.bg, border: `2px solid ${P.teal.border}`, borderRadius: '20px', boxShadow: `4px 4px 0 ${P.teal.shadow}`, p: 3, mb: 3 }}>
            <CharacterMessage speaker="Ms. Mabrouki" message={results.passed ? "Outstanding! You've demonstrated strong B1-level skills in social media language and grammar. Well done!" : "You're making progress, but need more practice. Review the feedback and try again. You've got this!"} />
          </Box>
