...
export default function RemedialC1TaskG() {
  ...
  React.useEffect(() => { window.__remedialSkip = () => navigate('/phase4/step/2/remedial/c1/taskH') }, [])
      ...
      <Box sx={{ minHeight: '100vh', bgcolor: P.pageBg, py: 4 }}>
        <Container maxWidth="md">
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <Box component="button" onClick={() => navigate('/phase4/step/2/remedial/c1/taskH')} sx={{ ...cardSx('blue'), width: '100%', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: '1rem', '&:hover': { transform: 'translate(-2px,-2px)', boxShadow: `6px 6px 0 ${P.blue.shadow}` } }}>
              Continue to Task H →
            </Box>
          </motion.div>
        </Container>
