...
export default function RemedialB1Results() {
  ...
  const handleRedirect = () => {
    ...
    if (scores.passed) navigate('/phase4/step/2/remedial/b2/taskA')
    else navigate('/phase4/step/2/remedial/b1/taskA')
  }
      ...
      <Container maxWidth="md">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <Box sx={{ ...cardSx(resultColor), mb: 3, textAlign: 'center' }}>
            <Typography variant="h4" fontWeight="bold" gutterBottom>Phase 4 - Step 2: Remedial Activities</Typography>
          ...
          <Box sx={{ ...cardSx(resultColor), mb: 3, textAlign: 'center' }}>
            ...
            <Box sx={{ ...cardSx(scores.passed ? 'green' : 'red'), mt: 2 }}>
              <Typography variant="h6" fontWeight="bold">
                {scores.passed ? '✅ You have passed Step 2 Remedial B1!' : '❌ Score below passing threshold'}
              </Typography>
          ...
          <Box sx={{ ...cardSx('yellow'), textAlign: 'center' }}>
            ...
            {devMode && (
              <Box
                component="button"
                onClick={() => navigate('/phase4/step/2/remedial/b2/taskA')}
                sx={{
                  mt: 2, cursor: 'pointer', px: 4, py: 1.5, fontSize: '0.9rem', fontWeight: 'bold',
                  bgcolor: '#1a1a2e', color: '#00ff88', border: '2px dashed #00ff88',
                  borderRadius: '12px', '&:hover': { opacity: 0.8 }
                }}
              >
                ⚡ DEV: Skip to B2
              </Box>
