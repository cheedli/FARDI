...
export default function RemedialB2TaskA() {
  ...
  React.useEffect(() => { window.__remedialSkip = () => navigate('/phase4_2/step/4/remedial/b2/taskB') }, [])
        ...
        <Box>
          ...
          {allFilled && (
            <Box component="button" onClick={() => handleSendMessage(message.id)} sx={{
              ...
              px: 2, py: 0.75, fontWeight: 700, fontSize: '0.85rem', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 0.5,
        ...
        <Container maxWidth="md">
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
            <Box sx={{ bgcolor: P.orange.bg, border: `2px solid ${P.orange.border}`, borderRadius: '20px', boxShadow: `4px 4px 0 ${P.orange.shadow}`, p: 3, mb: 3 }}>
              ...
              <Typography variant="h6" sx={{ color: P.orange.shadow }}>Level B2 - Task A: Role-Play Dialogue Complete!</Typography>
            </Box>
            <Box sx={{ bgcolor: passed ? P.green.bg : P.red.bg, border: `2px solid ${passed ? P.green.border : P.red.border}`, borderRadius: '20px', boxShadow: `4px 4px 0 ${passed ? P.green.shadow : P.red.shadow}`, p: 4, textAlign: 'center', mb: 3 }}>
              <Typography variant="h3" sx={{ fontWeight: 'bold', color: passed ? P.green.shadow : P.red.shadow }}>{score} / 5</Typography>
            ...
            <Box sx={{ textAlign: 'center' }}>
              ...
              }}>Continue to Task B: Writing</Box>
            </Box>
          </motion.div>
        </Container>
      </Box>
    ...
    <Box sx={{ minHeight: '100vh', bgcolor: P.pageBg, py: 4 }}>
      <Container maxWidth="md">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
          ...
          <Box sx={{ bgcolor: P.orange.bg, border: `2px solid ${P.orange.border}`, borderRadius: '20px', boxShadow: `4px 4px 0 ${P.orange.shadow}`, p: 3, mb: 3 }}>
            <Typography variant="h5" sx={{ fontWeight: 'bold', color: P.orange.shadow }}>Phase 4.2 - Step 4: Remedial Activities</Typography>
            <Typography variant="h6" sx={{ color: P.orange.shadow }}>Level B2 - Task A: Role-Play Dialogue</Typography>
          </Box>

          <Box sx={{ mb: 3 }}>
            <CharacterMessage speaker="Ms. Mabrouki" message="Welcome to the Role-Play Dialogue! Complete the conversation by choosing the correct words from the word bank. Tap words to fill in the blanks, then send each message!" />
          </Box>
          ...
          <Box sx={{ bgcolor: '#E5DDD5', borderRadius: '16px', overflow: 'hidden', border: `2px solid ${P.teal.border}`, boxShadow: `4px 4px 0 ${P.teal.shadow}`, mb: 3 }}>
            <Box sx={{ bgcolor: '#075E54', color: 'white', p: 1.5, display: 'flex', alignItems: 'center', gap: 2 }}>
              ...
              <Box component="span" sx={{ bgcolor: 'rgba(255,255,255,0.2)', color: 'white', borderRadius: '999px', px: 1.5, py: 0.25, fontSize: '0.8rem', fontWeight: 700 }}>{score}/5</Box>
            </Box>
            ...
            <Box sx={{ p: 2, minHeight: 300, maxHeight: 450, overflowY: 'auto' }}>
              {displayedMessages.map((message, index) => (
                <Box key={index} sx={{ display: 'flex', justifyContent: message.sender === 'YOU' ? 'flex-end' : 'flex-start', mb: 1.5, alignItems: 'flex-end' }}>
                  ...
                  <Box sx={{ maxWidth: '70%', p: 1.5, borderRadius: message.sender === 'YOU' ? '8px 8px 0 8px' : '8px 8px 8px 0', bgcolor: message.sender === 'YOU' ? '#DCF8C6' : 'white', boxShadow: '0 1px 3px rgba(0,0,0,0.12)' }}>
                    {message.sender !== 'YOU' && (
