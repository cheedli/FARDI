...
const WORD_BANK_ORIGINAL = ['the', 'a', 'be', 'exposed', 'that', 'register', 'is', 'successful']
...
export default function Phase4_2Step5RemedialC1TaskA() {
  ...
  const handleComplete = () => {
    ...
    fetch('/api/phase4/remedial/log', { method: 'POST', headers: { 'Content-Type': 'application/json' }, credentials: 'include', body: JSON.stringify({ phase: '4.2', level: 'C1', task: 'A', step: 5, score, max_score: 8, completed: true, time_taken: 0 }) }).catch(err => console.error('Log error:', err))
  }
  ...
  const renderMessageContent = (message) => {
    ...
    if (message.type === 'response') {
      if (message.sent) {
        ...
        return <Typography variant="body1" sx={{ fontWeight: 500 }}>{parts.map((part, idx) => (<React.Fragment key={idx}>{part}{idx < parts.length - 1 && <strong style={{ color: P.purple.shadow }}>{answers[`msg${message.id}_blank${idx}`]}</strong>}</React.Fragment>))}</Typography>
      }
      const parts = message.template.split(/_{3,}/)
              ...
              <React.Fragment key={idx}>
                ...
                {idx < parts.length - 1 && (
                  <Box component="span" onClick={() => handleBlankClick(message.id, idx)} sx={{
                    bgcolor: answers[`msg${message.id}_blank${idx}`] ? P.purple.bg : (isDark ? '#333' : '#fff'),
                    border: `2px dashed ${answers[`msg${message.id}_blank${idx}`] ? P.purple.border : '#999'}`,
                    borderRadius: '999px', px: 1.5, py: 0.3, fontSize: '0.8rem', fontWeight: 700,
                    color: answers[`msg${message.id}_blank${idx}`] ? P.purple.shadow : '#666',
        ...
        <Container maxWidth="md">
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
            <Box sx={{ bgcolor: P.purple.bg, border: `2px solid ${P.purple.border}`, borderRadius: '20px', boxShadow: `4px 4px 0 ${P.purple.shadow}`, p: 3, mb: 3 }}>
              <Typography variant="h4" gutterBottom fontWeight="bold" sx={{ color: P.purple.shadow }}>Phase 4.2 - Step 5: Remedial Activities</Typography>
              <Typography variant="h5" sx={{ color: P.purple.border }}>Level C1 - Task A: Debate Simulation Complete!</Typography>
            </Box>
            <Box sx={{ bgcolor: P.yellow.bg, border: `2px solid ${P.yellow.border}`, borderRadius: '20px', boxShadow: `4px 4px 0 ${P.yellow.shadow}`, p: 4, textAlign: 'center', mb: 3 }}>
              <Typography variant="h3" gutterBottom fontWeight="bold" sx={{ color: P.yellow.shadow }}>{score} / 8</Typography>
    ...
    <Box sx={{ minHeight: '100vh', bgcolor: P.pageBg, py: 4 }}>
      <Container maxWidth="md">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
          ...
          <Box sx={{ bgcolor: P.purple.bg, border: `2px solid ${P.purple.border}`, borderRadius: '20px', boxShadow: `4px 4px 0 ${P.purple.shadow}`, p: 3, mb: 3 }}>
            <Typography variant="h4" gutterBottom fontWeight="bold" sx={{ color: P.purple.shadow }}>Phase 4.2 - Step 5: Remedial Activities</Typography>
            <Typography variant="h5" gutterBottom sx={{ color: P.purple.border }}>Level C1 - Task A: Debate Simulation</Typography>
          ...
          <Box sx={{ maxWidth: 600, mx: 'auto', bgcolor: isDark ? '#1a1a2e' : '#E5DDD5', borderRadius: '20px', overflow: 'hidden', border: `2px solid ${P.purple.border}`, boxShadow: `4px 4px 0 ${P.purple.shadow}` }}>
            <Box sx={{ bgcolor: '#075E54', color: 'white', p: 1.5, display: 'flex', alignItems: 'center', gap: 2 }}>
              ...
              <Box component="span" sx={{ bgcolor: 'rgba(255,255,255,0.2)', borderRadius: '999px', px: 2, py: 0.3, fontSize: '0.85rem', fontWeight: 700, color: 'white' }}>{score}/8</Box>
            </Box>
              ...
              <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
                {wordBank.map((word, idx) => (
                  <Box key={idx} component="span" onClick={() => handleWordClick(word)} sx={{
                    bgcolor: selectedWord === word ? P.purple.bg : (isDark ? '#333' : 'white'),
                    border: `2px solid ${selectedWord === word ? P.purple.border : '#D1D1D1'}`,
                    borderRadius: '999px', px: 2, py: 0.3, fontSize: '0.85rem', fontWeight: selectedWord === word ? 700 : 600,
                    color: selectedWord === word ? P.purple.shadow : (isDark ? '#ccc' : '#2c3e50'),
