...
export default function Phase5Step1RemedialB2Results() {
  ...
  const [nextUrl, setNextUrl] = useState('/phase5/subphase/1/step/1/remedial/b2/task/a')
  ...
  const calculateFinalScore = async () => {
    ...
    try {
      const result = await phase5API.calculateRemedialScore(1, 'B2', {
      ...
      setNextUrl(backendNextUrl || '/phase5/subphase/1/step/1/remedial/b2/task/a')
    } catch (error) {
      ...
      <Container maxWidth="md">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
          ...
          <Box sx={{
            ...
            <Typography variant="h5" gutterBottom color={P.orange.shadow}>Step 1: Remedial Practice - Level B2</Typography>
          ...
          <Box sx={{
            ...
            <Stack spacing={2} sx={{ mt: 2 }}>
              {[
                { label: 'Task A (Role-Play)', score: scores.taskA, max: 7 },
                { label: 'Task B (Writing)', score: scores.taskB, max: 8 },
                { label: 'Task C (Matching)', score: scores.taskC, max: 8 },
                { label: 'Task D (Spell & Explain)', score: scores.taskD, max: 6 },
                { label: 'Task E (Conditionals)', score: scores.taskE, max: 6 },
                { label: 'Task F (Passives)', score: scores.taskF, max: 6 },
              ].map((item, i) => (
                <Box key={i}>
