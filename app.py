...
def is_ai_generated(text):
    """
    Vérifie si le texte est généré par une IA en utilisant des heuristiques simples.
    Dans une version de production, vous utiliseriez une API comme Sapling ou un modèle plus robuste.
    
    Retourne un tuple (est_généré_par_ia, score, détails).
    """
    # Pour une application réelle, implémentez ici l'appel à une API comme Sapling
    
    # Ne pas analyser les textes trop courts
    if len(text) < 50:
        return (False, 0, "Text too short for reliable detection")
    
    # Initialisation des scores et indicateurs
    total_score = 0
    reasons = []
    ...
    if phrases_found >= 2:
        total_score += min(0.25 * phrases_found / 3, 0.25)  # Plafonné à 0.25
        reasons.append(f"Contains {phrases_found} phrases often used by AI")
    
    # 3. Analyser la structure des phrases
    sentences = re.split(r'[.!?]+', text)
    sentences = [s.strip() for s in sentences if s.strip()]
    
    if len(sentences) > 3:
        # Calculer la variabilité de longueur des phrases
    ...
    # 4. Analyser la diversité du vocabulaire (TTR - Type-Token Ratio)
    ...
    # 5. Vérifier la cohérence des n-grammes (triplets de mots)
    if len(words) > 20:
        # Créer des triplets de mots
        triplets = [' '.join(words[i:i+3]) for i in range(len(words)-2)]
        # Compter les occurrences
        triplet_counts = Counter(triplets)
        # Vérifier si certains triplets sont répétés
        repeated_triplets = sum(1 for count in triplet_counts.values() if count > 1)
        
        if repeated_triplets == 0 and len(triplets) > 10:
            # Les textes humains ont tendance à répéter certains motifs
            total_score += 0.1
            reasons.append("No repeated phrase patterns (unusual for human writing)")
    
    # Arrondir le score pour plus de lisibilité
    final_score = min(round(total_score, 2), 1.0)
    
    # Considérer comme généré par IA si le score est supérieur à 0.5
...
def get_ai_feedback():
    ...
    # Vérifier si la réponse a été générée par une IA
    is_ai, ai_score, ai_reasons = is_ai_generated(response)
    
    # D'abord, exécuter une évaluation rapide pour identifier le niveau et les problèmes
    ...
    # Ajouter des informations sur la détection d'IA si nécessaire
    if is_ai:
        improvements = ["Votre réponse semble utiliser des formulations typiques de l'IA"] + improvements
    
    # Créer un prompt de coaching basé sur l'évaluation
    coaching_notes = ""
    ...
    # Créer un prompt basé sur la question, la réponse et les notes de coaching
    prompt = f"""
    ...
    # Renvoyer également les données d'évaluation pour les fonctionnalités de l'interface utilisateur
    return jsonify({
...
def submit_response():
    ...
    # Vérifier si la réponse a été générée par une IA
    is_ai, ai_score, ai_reasons = is_ai_generated(player_response)
    
    # Si détecté comme généré par IA, afficher un message mais continuer le processus
    if is_ai:
        flash(f"Attention: Votre réponse a été détectée comme potentiellement générée par une IA (score: {ai_score:.2f}). Les réponses personnelles sont encouragées pour un meilleur apprentissage.", "warning")
        
        # Stocker l'information dans la session pour référence
    ...
    if current_step < len(DIALOGUE_QUESTIONS):
        ...
        # Calculer et attribuer des XP
        xp_earned = question_data.get('xp_reward', 10)
        # XP bonus en fonction du niveau
        ...
        # Réduire les XP si générée par IA
        if is_ai:
            xp_earned = int(xp_earned * 0.5)  # 50% des XP normaux
        ...
        # Afficher un message sur les XP gagnés
        if is_ai:
            flash(f"Réponse soumise ! Vous avez gagné {xp_earned} XP (réduit car détectée comme générée par IA).", "success")
        else:
            flash(f"Réponse soumise ! Vous avez gagné {xp_earned} XP.", "success")
